...
"""Deterministic trade auditor for futures recommendations.

The auditor is the non-LLM trade gate used by the portfolio manager. It reads
analyst signals, market confirmation, and recent attribution feedback, then
decides whether a proposed exposure should be allowed, reduced, blocked, or
held. It does not select analyst agents, calculate lots, or place orders.
"""
...
class TradeAuditorInput(BaseModel):
    """Input payload for the deterministic futures trade auditor."""
...
def _infer_horizon_from_payload(payload: "TradeAuditorInput", target_side: str) -> str:
...
def _infer_market_regime_from_payload(payload: "TradeAuditorInput", target_side: str) -> str:
    target_signal = _target_signal_for_side(target_side)
    for item in payload.analyst_signals or []:
        if not isinstance(item, dict) or _signal_text(item.get("signal")) != target_signal:
            continue
...
class AttributionFeedbackCalibrator:
    """Rule-based attribution feedback used by the trade auditor."""

    def __init__(self, full_config: Dict[str, Any]):
        self.full_config = full_config or {}
        self.auditor_config = self.full_config.get("trade_auditor") or self.full_config.get("decision_planner", {}) or {}
        self.feedback_config = self.auditor_config.get("attribution_feedback", {}) or {}
        self.cold_start_config = self.auditor_config.get("cold_start", {}) or {}
    ...
    def calibrate(self, payload: TradeAuditorInput) -> Dict[str, Any]:
    ...
    def _evaluate_conditional_combo(self, payload: TradeAuditorInput) -> Dict[str, Any]:
        target_side = _target_side_from_ratio(payload.raw_position_ratio)
    ...
    def _evaluate_cold_start(self, payload: TradeAuditorInput) -> Dict[str, Any]:
...
class TradeAuditor:
    """Deterministic futures trade auditor."""
    ...
    @property
    def enabled(self) -> bool:
        return bool(self.auditor_config.get("enabled", False))

    def plan(self, payload: TradeAuditorInput) -> TradeAuditorOutput:
        target_side = _target_side_from_ratio(payload.raw_position_ratio)
        policy_version = str(self.auditor_config.get("policy_version", "deterministic_v1"))
        learning_mode = str(self.auditor_config.get("learning_mode", "audit_only"))
        reasons: List[str] = []
        notes: List[str] = []
        diagnostics: Dict[str, Any] = {
        ...
        if not self.enabled:
            return self._output(
                ...
                multiplier=1.0,
                reasons=["trade_auditor_disabled"],
                notes=["trade auditor disabled; legacy controls may apply"],
                diagnostics=diagnostics,
                payload=payload,
        ...
        if target_side not in {"long", "short"}:
            return self._output(
                ...
                multiplier=0.0,
                reasons=["flat_target"],
                notes=["auditor received flat target; no new exposure required"],
                diagnostics=diagnostics,
                payload=payload,
                policy_version=policy_version,
                learning_mode=learning_mode,
        ...
        if not _is_new_or_increasing_exposure(payload.raw_position_ratio, payload.current_position_ratio):
            return self._output(
                ...
                notes=["auditor skipped blocking because target is not new or increasing exposure"],
                diagnostics=diagnostics,
                payload=payload,
                policy_version=policy_version,
                learning_mode=learning_mode,
        ...
        return self._output(
            ...
            target_side=target_side,
            multiplier=multiplier,
            reasons=_dedupe(reasons) or ["trade_auditor_allow"],
            notes=notes,
            diagnostics=diagnostics,
            payload=payload,
            policy_version=policy_version,
    ...
    def _evaluate_market_confirmation(self, payload: TradeAuditorInput) -> Dict[str, Any]:
...
# Backward-compatible names for historical tests, snapshots, and user scripts.
DecisionPlannerInput = TradeAuditorInput
DecisionPlannerOutput = TradeAuditorOutput
LightweightDecisionPlanner = TradeAuditor

__all__ = [
    "TradeAuditor",
    "TradeAuditorInput",
    "TradeAuditorOutput",
    "DecisionPlannerInput",
