...
class FuturesDailySettlement:
    ...
    def run_phase3(self, config_id: str, trading_date: Any) -> FuturesSettlementRecord:
        ...
        settle_prices = self._fetch_contract_settle_prices(ledgers, transactions, trading_dt)
        ...
        logger.info(
            ...
            f"balance={official_portfolio.cashflow:,.2f}, pnl={settlement_summary['daily_pnl']:+,.2f}, "
    ...
    def _finalize_ledgers(
        ...
        current_balance = previous_balance + previous_margin - current_margin + total_daily_pnl - total_commission
        margin_available = current_balance - total_margin
        margin_ratio = total_margin / current_balance if current_balance > 0 else (1.0 if total_margin > 0 else 0.0)
