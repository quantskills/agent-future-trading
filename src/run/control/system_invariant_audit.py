...
"""Fail-fast runtime invariant audit for completed backtest windows.

This command does not evaluate profitability and does not participate in trade
generation. It only checks whether persisted recommendations, transactions,
intraday decisions, and action-values obey the system contracts required before
backtest results can be treated as strategy results.
"""
...
def main() -> int:
    ...
    payload["contract_version"] = "agentquant.system_invariants.v1"
    if args.json:
