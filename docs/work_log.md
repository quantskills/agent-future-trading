...
（8）收口 PM 机制口径到端到端业务链的 rank 清理字段边界。修改：`final_action_semantics.py`、`workflow.py`、`pm_opportunity_ranking.py`、统一字段语义表和回归测试。原因：2025-03-27 C/HC/M 暴露出非 rank 动作消费了生命周期学习，但 workflow/canonicalize 把 `lifecycle_learning_trace`、`learning_impact_delta` 当作 rank trace 字段清理，导致 PG hard fail `lifecycle_learning_trace_missing`。本次拆分 rank 专属清理字段与生命周期学习字段：非 full-market rank 清理只删除 `opportunity_rank/rank_source/rank_scope/capital_rank_generated_by/rank_capital_role/capital_layer/capital_ratio_source/rank_reason/rank_input_components`，保留 `lifecycle_learning_trace/learning_impact_delta/pm_lifecycle_learning_trace/pm_lifecycle_learning_impact_delta`；走 rank 的合约仍必须同时具备 rank trace 和 lifecycle trace，不降低 PG hard fail，不改 11 个智能体边界、不改仓位参数、不改 Trader/Accountant/Researcher 主职责。

（9）收口 PM 六步机制与 workflow 编排边界一比一实现。修改：`portfolio_manager.py`、`workflow.py`、新增 `pm_ticker_side_selection.py`、`pm_lifecycle_action_port.py`、`pm_lifecycle_learning_router.py`、`pm_full_market_capital_deployment.py`，删除旧 `pm_opportunity_ranking.py`，同步 `final_action_semantics.py`、`pg_contract_coverage_audit.py`、统一字段语义表、PM/workflow 机制文档和回归测试。原因：`docs/mechanism_pm.md` 和 `docs/mechanism_workflow.md` 已明确 PM 才是组合决策者，workflow 只是运行编排层；本次把单品种方向选择迁入 `pm_ticker_side_selection`，把唯一全市场 rank 与资金部署迁入 PM 全市场资金部署工具，并通过 `portfolio_manager.finalize_pm_full_market_contracts()` 作为 PM 边界出口供 workflow 保存已签出结果。workflow 不再生成 rank、不再做预算部署、不再改 final_action_contract、不再补字段、不再保留 atomic fallback；缺合约、非法合约或未排名新增风险继续由 PM 自检与 PG hard fail 阻断。保留 final_action_contract 唯一事实、PG hard fail、Trader/Accountant/Researcher 边界、PM artifact 安全摘要、lifecycle trace/rank trace 语义、0.008 probe、20% 总保证金和 0.5 净敞口预算线。

（10）补完 PM 六步主链强制入口与一次性最终合约落库。修改：`portfolio_manager.py`、`workflow.py`、`pm_lifecycle_learning_router.py` 和阶段流回归测试。原因：第 2 步 `pm_lifecycle_action_port` 与第 4 步 `pm_lifecycle_learning_router` 不能只作为新增工具存在，必须成为 PM 主链统一入口；本次在最终合约生成前强制分类生命周期动作口，并按动作口过滤 action-value 学习，只有匹配生命周期的学习进入最终合约和后续 rank/持仓/释放资金决策，execution 学习仍只进入 trigger/profile trace。workflow 也改为先收齐所有 PM 候选，调用 PM finalization 完成第 5、6 步，再 `save_futures_recommendation()` 一次性保存最终 PM 合约；不再先保存 PM 初稿再 update。Auditor 后续 update 只保存独立审计结果，不参与 PM 策略合约改写。

==========2026年07月06日==========

（1）新增唯一全市场 rank 评分策略配置入口。修改：新增 `src/config/rank_score_policy.yaml`，同步 `dev.yaml`、`config_normalizer.py`、`pm_signal_fusion.py`、`pm_full_market_capital_deployment.py`、`parameter.md`、`mechanism_pm.md`、统一字段语义表和回归测试。原因：rank 评分机制需要能在 40 个干净交易日后依据 rank 分层平均收益微调，但不能再通过改代码硬编码权重完成；本次把冷启动证据质量、资金层级资格、open/add action-value 学习修正、产品/setup/trigger 历史表现、trigger/execution 质量、冲突/风险/失效边界惩罚和资金效率小修正的权重集中到受控 catalog。该配置只影响 `rank_score/rank_score_components` 和资金效率小修正，不新增第二套 rank，不创建交易权限，不改仓位参数，不覆盖 `0.008` probe、`20%` 总保证金、`0.5` 净敞口或单品种预算，不改 PM/Trader/Accountant/Researcher 边界。

（2）收口 PM 六步主链顺序和工具职责边界。修改：`portfolio_manager.py`、`pm_signal_fusion.py`、`pm_ticker_side_selection.py`、`pm_lifecycle_learning_router.py`、`pm_full_market_capital_deployment.py`、`unified_field_semantics.md` 和回归测试。原因：PM 第 2 步生命周期动作口不能晚于 scorecard/side selection；本次在 scorecard 前先构造初始生命周期意图，scorecard 只消费 PM 信号融合组件，最终生命周期允许/拒绝/trigger-profile 分流由 `pm_lifecycle_learning_router` 统一落入最终合约 trace。`pm_ticker_side_selection` 只保留 `side_priority/ticker_side_priority/side_priority_score/candidate_quality/candidate_layer_hint`，不再输出 `rank_score/capital_priority_score/capital_priority_tier/capital_layer` 等最终资金 rank 语义；`pm_signal_fusion` 只输出证据、候选质量和 rank 候选输入组件，最终 `rank_score/rank_score_components/opportunity_rank` 只能由 PM 第 5 步 `pm_full_market_capital_deployment` 写入。验证：指定单测集合 445 tests OK，`compileall src` 通过；不改仓位参数、不恢复 workflow rank、不恢复 atomic fallback、不改 Trader/Accountant/Researcher 边界、不降低 PG hard fail。

（3）收口 PM 第 3 步方向选择生成权。修改：`pm_signal_fusion.py`、`pm_contract_builder.py`、`unified_field_semantics.md` 和回归测试。原因：`signal_collector` 与 `pm_signal_fusion` 只能保真整理分析师结构化方向证据，不能提前生成 PM 内部 `side_priority/ticker_side_priority`；本次把 fusion 输出改为 `analyst_direction_evidence/direction_evidence_strength/direction_evidence_components`，保留方向、证据强弱、setup、trigger、invalidation、冲突和学习校准输入，唯一 `side_priority/ticker_side_priority/side_priority_score` 生成口收束到 `pm_ticker_side_selection.py`。`pm_contract_builder` 只携带已经由 PM 方向选择工具落地的 side priority，不再从 raw scorecard 写出空的方向优先级伪字段；不改仓位参数、不恢复 workflow rank、不改 Trader/Accountant/Researcher 边界、不降低 PG hard fail。

（4）迁移 PM 信号融合工具命名和目录边界。修改：删除 `src/tools/agent_tools/analysis/analyst_signal_fusion.py`，新增 `src/tools/agent_tools/decision/pm_signal_fusion.py`，同步 `portfolio_manager.py`、`pm_ticker_side_selection.py`、契约覆盖审计、机制文档、统一字段语义表和回归测试导入。原因：该工具真实使用者是 PM，用于生成 PM 侧 `opportunity_scorecard`、候选质量和 rank 候选输入组件，不是 `signal_collector` 的证据收集工具；信号收集员仍只调用 `signal_evidence_collection.py`。本次不保留旧 wrapper，不改变仓位参数、不恢复 workflow rank、不改 Trader/Accountant/Researcher 边界、不降低 PG hard fail。

（5）收口 PM 第 4 步生命周期学习路由唯一入口。修改：`portfolio_manager.py`、`pm_lifecycle_learning_router.py`、`pm_contract_builder.py` 和回归测试。原因：PM 早期 scorecard 与最终合约前不能各自形成一套学习路由；本次删除 `_route_pm_scorecard_action_values`，`portfolio_manager.py` 主链只保留一次 `route_lifecycle_learning()` 调用，router 固定输出 `decision_learning_rows/trigger_profile_learning_rows/rejected_learning_rows` 三类结果。`execution` action-value 必须进入 `trigger_profile_learning` trace，不能进入 rejected，不能生成 `open/add` rank delta；最终合约同步写出 `trigger_profile_learning`、`trigger_profile_indices` 和 `execution_profile_learning_direct_to_rank=false`。不改仓位参数、不恢复 workflow rank、不恢复 atomic fallback、不改 Trader/Accountant/Researcher 边界、不降低 PG hard fail。

（6）收口 PM 第 2 步生命周期动作口与最终合约自检边界。修改：`portfolio_manager.py`、`pm_lifecycle_action_port.py`、`pm_contract_builder.py`、`pm_contract_self_check.py`、统一字段语义表和回归测试。原因：PM 主链只能有一个 `primary_lifecycle_action_port`，它必须在 scorecard、方向选择和学习路由前生成；最终合约前的分类只能作为 `contract_lifecycle_self_check`，用于校验最终 `current_lots/target_lots/final_action` 与主口是否一致，不得重新决定学习路由或生成第二套生命周期语义。若预算不足、hard risk、无触发或还原 wait 导致 final contract 与 primary port 不一致，必须写入 `lifecycle_port_transition_reason`；无合法原因的 transition 由 `pm_contract_self_check` fail。保持第 4 步 router 只读取 primary port，不改仓位参数、不恢复 workflow rank、不改 Trader/Accountant/Researcher 边界、不降低 PG hard fail。

（7）同步 PM 第 2.1 动作口工具文档边界。修改：`mechanism_pm.md` 和决策工作流文档静态测试。原因：PM 第 2 步主工具必须固定为 `pm_lifecycle_action_port.py`，`final_action_semantics` 只能作为共享语义依赖，`pm_contract_builder.py` 只在第 6 步生成唯一 `final_action_contract`，不得参与动作口判断；本次用测试锁住第 2.1 与总纲一致，避免后续再把动作口判断放回 builder 或旧 common 口径。

（8）结构性重写 PM 六步主链签约时点。修改：`portfolio_manager.py`、`pm_full_market_capital_deployment.py` 和决策工作流回归测试。原因：单品种 PM 主链此前仍在全市场资金 rank 前调用 `pm_contract_builder` 生成 `final_action_contract`，随后第 5 步再改已签合约，违背 `mechanism_pm.md` 的“候选生成 -> rank/部署 -> 最终签约”顺序。本次把单品种阶段改为只生成 `pm_internal_candidate`，第 5 步 `pm_full_market_capital_deployment` 只写 `pm_capital_deployment_decision`，不创建、修复或改写最终合约；若历史路径误把已签 `final_action_contract` 传入第 5 步，部署工具也只能写部署决定，不能回写该合约。第 6 步由 `portfolio_manager.finalize_pm_full_market_contracts()` 统一调用 `pm_contract_builder` 和 `pm_contract_self_check` 签唯一最终合约。同步移除 PM 主链中 exact/similar/policy 学习检索后的 scorecard/side selection 重跑，保证 `build_opportunity_scorecard()`、`select_ticker_side()` 和 `route_lifecycle_learning()` 在业务主链各只执行一次。不改仓位参数、不恢复 workflow rank、不恢复 atomic fallback、不降低 PG hard fail、不改 Trader/Accountant/Researcher 边界。

（9）收口 PM 双路径六步 runner 与第 6 步唯一签约。修改：`portfolio_manager.py`、`pm_full_market_capital_deployment.py`、`pg_contract_coverage_audit.py` 和回归测试。原因：PM 正常主链已按 `1->2->3->4->5/6` 运行，但异常早退、风控阻断、数据缺失和 hard block 仍可能通过最小合约路径绕过第 6 步签约。本次删除 `_build_minimal_final_action_contract()` 及其调用点，所有路径统一先生成 `pm_internal_candidate`，通过 `candidate_status=blocked/normal/demoted/budget_rejected` 区分状态，再由 `_sign_pm_candidate_recommendation()` 调用 `pm_contract_builder.py` 与 `pm_contract_self_check.py` 写入唯一 `final_action_contract`。`pm_full_market_capital_deployment.py` 继续只返回 `pm_capital_deployment_decision`，不读取、不修复、不规范化已签合约；契约覆盖审计同步识别第 6 步 signer 与 PM contract builder 为唯一生产路径。不改仓位参数、不恢复 workflow rank、不恢复 atomic fallback、不降低 PG hard fail、不动 Trader/Accountant/Researcher 边界、不破坏 PM artifact 安全摘要。

（10）收口 workflow 编排边界与虚拟组合投影语义。修改：`workflow.py` 和阶段流回归测试。原因：`workflow` 必须一比一符合 `mechanism_workflow.md`，只负责调度、传递、保存、触发和阻断，不能被函数名或旧分支误解为资金部署、合约修复或策略 fallback。本次把 `_apply_virtual_recommendation_to_portfolio()` 改为 `_project_signed_contract_to_virtual_portfolio()`，只读取 PM 已签 `final_action_contract` 投影 Phase1 虚拟组合；没有最终合约或缺 `target_lots` 时直接跳过投影，不从 `recommendation.action/lots` 推导手数，不生成或补齐合约。`workflow.py` 继续只调用 PM 边界出口 `finalize_pm_full_market_contracts()`，不直接调用 PM rank/资金部署工具，不写 `opportunity_rank`、`capital_deployment`，不恢复 atomic fallback，不改仓位参数、不降低 PG hard fail、不动 Trader/Accountant/Researcher 边界。

（11）收口 workflow 智能体输出零生产边界。修改：`workflow.py`、`signal_collector.py`，新增 `signal_collection_data_unavailable.py`，同步阶段流回归测试。原因：`workflow` 不能生产任何智能体输出，缺盘前基准价时也不能自己构造 `AnalystSignal`、`signal_snapshot` 或 skipped strategy recommendation；本次删除 workflow 内 `_build_missing_pre_open_reference_signals()` 和 `_build_signal_snapshot_from_signals()`，`_coerce_phase1_recommendation()` 改为只接受 PM 已签 recommendation，PM 未签则 hard fail。盘前基准价缺失时，workflow 只设置上下文缺失标志并调度 `signal_collector`，由 `signal_collector` 边界通过确定性工具签出 `data_unavailable_no_trade` 结构化信号包，再交 PM 签唯一合约；不恢复 workflow rank、不恢复资金部署、不改 PM 六步、不改字段语义、不降低 PG hard fail、不动 Trader/Accountant/Reviewer/Researcher 边界。
