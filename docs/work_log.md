...
（1）新增唯一全市场 rank 评分策略配置入口。修改：新增 `src/config/rank_score_policy.yaml`，同步 `dev.yaml`、`config_normalizer.py`、`analyst_signal_fusion.py`、`pm_full_market_capital_deployment.py`、`parameter.md`、`mechanism_pm.md`、统一字段语义表和回归测试。原因：rank 评分机制需要能在 40 个干净交易日后依据 rank 分层平均收益微调，但不能再通过改代码硬编码权重完成；本次把冷启动证据质量、资金层级资格、open/add action-value 学习修正、产品/setup/trigger 历史表现、trigger/execution 质量、冲突/风险/失效边界惩罚和资金效率小修正的权重集中到受控 catalog。该配置只影响 `rank_score/rank_score_components` 和资金效率小修正，不新增第二套 rank，不创建交易权限，不改仓位参数，不覆盖 `0.008` probe、`20%` 总保证金、`0.5` 净敞口或单品种预算，不改 PM/Trader/Accountant/Researcher 边界。

（2）收口 PM 六步主链顺序和工具职责边界。修改：`portfolio_manager.py`、`analyst_signal_fusion.py`、`pm_ticker_side_selection.py`、`pm_lifecycle_learning_router.py`、`pm_full_market_capital_deployment.py`、`unified_field_semantics.md` 和回归测试。原因：PM 第 2 步生命周期动作口不能晚于 scorecard/side selection；本次在 scorecard 前先构造初始生命周期意图并调用 `pm_lifecycle_action_port`，随后用 `pm_lifecycle_learning_router` 过滤 scorecard 输入，`execution` action-value 明确路由到 `trigger_profile_learning` 而不是 rejected 或 open/add rank delta。`pm_ticker_side_selection` 只保留 `side_priority/ticker_side_priority/side_priority_score/candidate_quality/candidate_layer_hint`，不再输出 `rank_score/capital_priority_score/capital_priority_tier/capital_layer` 等最终资金 rank 语义；`analyst_signal_fusion` 只输出证据、候选质量和 rank 候选输入组件，最终 `rank_score/rank_score_components/opportunity_rank` 只能由 PM 第 5 步 `pm_full_market_capital_deployment` 写入。验证：指定单测集合 445 tests OK，`compileall src` 通过；不改仓位参数、不恢复 workflow rank、不恢复 atomic fallback、不改 Trader/Accountant/Researcher 边界、不降低 PG hard fail。
