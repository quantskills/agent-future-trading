...
（6）[Researcher 逻辑交易日追溯] `research_learning.py`、Researcher 正式入口与 `researcher_learning.py` 将参考组合日期和研究来源交易日分开校验：recommendation 绑定的 `reference_portfolio_id` 必须属于正式 `Prev(T)`，持久化三份 AEC、SCC、recommendation、execution、transaction、settlement 与 Phase4 必须属于逻辑 `T`；Researcher 从真实 signal artifact 读取并共享校验 AEC，不再把 `signal.portfolio_id` 对应的组合日期当作信号日期。`source_trading_date=T` 的学习继续只能被目标逻辑日晚于 T 的正式检索消费。原因：修复 2025-03-26 研究阶段将 2025-03-25 参考组合误判为 45 份信号日期错误，同时不新增日期字段、别名或第二套日期语义。

（7）[条件 watch 进入既有资金竞争] `portfolio_manager.py` 删除排名前以自由文本否决已通过 canonical 结构校验的 `watch_for_trigger` 路径；`pm_reason_effects.py` 与 `final_action_semantics.py` 清除对应废弃 reason code，并使获选条件 FAC 的最终生命周期使用既有 `conditional_monitor`，同时保留 Step5 开仓 rank 生命周期和既有 `open_rank_*` 错误码。合法 watch 仍按原 rank、预算和 sizing 竞争，只有获选且获得手数时形成 `conditional_trigger_authority=true`、`requires_intraday_confirmation=true`、`can_execute_without_intraday_trigger=false` 的非零 FAC；Trader 继续以 15 分钟确认、1 分钟执行。原因：修复合法条件候选在排名前被清零而无法进入 Trader 的闭环，不改变 PM 六步、rank 公式、预算、sizing、策略参数或候选竞争机制。

（8）[逻辑 T 数据可见边界统一] `analyst_finoview_factors.py` 与 `router.py` 使用正式交易日机制解析 `Prev(T)` 及 catalog `release_lag_days`，Router 格式化基本面输入和 factor snapshot 共用同一个按 `tradeDate` 选择最新可见记录的选择器；未登记日期记录拒绝进入，`recordTime` 不被解释为发布时间。新闻盘前截止统一为正式 `Prev(T)`，更早有效新闻合法且无当日新增不阻断。`pandaai/api.py` 的分钟线除物理 cutoff 外还要求 provider `trading_date` 等于请求的逻辑 T，允许物理时间位于 Prev(T) 夜盘；既有 PandaAI 日线 `<T` 选择继续允许更早 T-n。原因：消除自然日、`<T` 和 release lag 并存的日期选择路径，防止未来事实或错误逻辑日分钟线进入策略与执行，同时不新增字段或默认事实。

（9）[三项缺陷失败优先回归与验收] `test_frozen_mainline_repairs.py`、`test_phase_flow_regression.py`、`test_reviewer_learning.py`、`test_market_confirmation.py`、`test_agent_contracts.py` 与 `test_pandaai_api_adapter.py` 增加 Prev(T)/T/Next(T) 追溯、错误 AEC 日期拒绝、完整条件 watch 到 15m/1m 执行、学习前视隔离、周末/正式交易日基本面新闻选择及夜盘分钟线逻辑日测试。旧生产代码先出现 1 个错误和 5 个失败；修复后相关定向集 23、90、312、99 项分别通过，全量 788 项通过，`compileall src`、`git diff --check` 及 2025-03-26 正式回测前门禁通过。生产 `--reset-config` 重跑在进程启动前被执行环境安全审批拒绝，数据库未重置，Phase1-Phase4、Researcher 与正式每日 PG 因而未执行、未登记为通过。原因：用行为证据闭环三项生产缺陷，并如实保留涉及外部 LLM 传输和本地状态重置的真实验收阻断。

（10）[已确认触发与条件执行分流] `portfolio_manager.py` 与 `trader_intraday_execution.py` 只用 canonical `trigger_valid/current_trigger_confirmed/invalidation_present/opportunity_state` 形成直执行权限：获 PM 资金授权的 probe/tradeable FAC 对 breakout、pullback、vwap_confirmed、event_immediate 统一写入并遵守 `can_execute_without_intraday_trigger=true`，Trader 不再用15分钟线复判，只选择合法1分钟线且记录 `trigger_checked=false`；条件 watch 继续保持三字段条件权限并使用15分钟触发、下一根合法1分钟执行。原因：避免已由正式事实确认的触发被 Trader 二次复判压死，同时不放宽条件 watch、行情缺失、Auditor 或市场规则边界。

（11）[add/scale 保证金投影同源] `final_action_semantics.py` 新增唯一内存计算函数 `project_margin_transition`，`auditor.py`、`pm_full_market_capital_deployment.py` 与 `trader.py` 共用 `current_account_margin-current_ticker_margin+target_ticker_margin` 及正增量公式；Trader 不再用目标品种总保证金与账户剩余额度比较而重复计算已有持仓，reduce/exit 继续不受新增风险保证金检查阻断。原因：统一 PM、Auditor、Trader 的硬保证金口径，不修改 rank、预算、sizing、目标手数或 FAC。

（12）[风险收缩证据隔离] `pm_signal_fusion.py` 与 `portfolio_manager.py` 保留 `risk_reduction_candidate` 独立计数和研究价值，但将其从新增风险 `supporting_signal_count`、tradeable/probe support、open/strong evidence、scorecard 与 rank 输入剔除；空仓风险收缩证据不能形成开仓权限，已有持仓仍只经既有生命周期形成 hold/reduce/exit，且不否决其他分析师的合法候选。原因：防止风险收缩证据被误用为新增风险资金与交易授权，同时不创建第二套持仓管理机制。

（13）[三类交易路径失败优先回归与门禁] 新增 `test_trade_path_incremental_repairs.py` 并接入回测前 `supported_business_paths`，覆盖四种合法直执行 profile、条件 watch、缺触发/失效/资金/Auditor 边界、long/short add/scale、多品种保证金、reduce/exit 和风险收缩隔离。旧实现首轮 11 项出现 11 个失败，底层 risk-reduction rank 资格补测再出现 1 个失败；修复后 12 项、相关主线 436 项、PG/状态组合 67 项、同库全链路 1 项、回测前验收模块 11 项及 ProtocolGovernor 5 项全部通过；全量 800 项、`compileall src`、`git diff --check` 和 2025-03-26 正式无 LLM 回测前十项门禁通过。原因：以当前生产函数和确定性行为测试证明三条路径闭环，不使用旧回测数据库或旧交易结果作为修复依据。

（14）[PM 方向与候选准入修复] `pm_ticker_side_selection.py` 将 Step2 代表方向收口为只读取 SCC 方向事实：无真实冲突时唯一 long/short 主方向写为 `preferred_side` 及优先级1，flat/mixed/conflicted 保持 flat；`pm_signal_fusion.py`、`portfolio_manager.py` 使完整且当前已触发的单来源候选保留真实低共识分进入既有 Step5 队列，另外两名分析师的 `no_opportunity` 不计支持也不构成否决。原因：修复方向优先级已形成但 preferred_side 仍为 flat，以及单来源候选在排名前被清零的断点；`rank_score_policy.yaml`、唯一 rank 实现、预算、sizing、策略参数和资金层均未修改。

（15）[证据缺口、硬数据与机会状态同源] `signal_evidence_collection.py` 新增唯一共享具体触发谓词并由 `analyst_quality.py`、`pm_signal_fusion.py` 共用；纯方向观点、单纯声明无触发或只等待未来周/月数据不能成为 watch，合法 watch 固定当前未触发，probe/tradeable 固定当前触发已确认。SCC 只汇总主方向合法状态，主方向全部 no-opportunity 时为 `not_applicable`；`missing_evidence/confirmation_requirements` 不再映射 `data_missing` 或按数量形成 `critical_data_gap`，PM 只消费共享 SCC 数据质量摘要且仅 `status=hard_fail` 硬阻断。三类分析师提示词同步同一语义。原因：分离证据不足、非每日基本面/新闻与真实硬数据故障，阻止 no-opportunity、反方向 watch 和自由文本触发污染候选支持。

（16）[四项缺陷失败优先回归与验收] `test_trade_path_incremental_repairs.py` 新增 Step2/Step5 分离、普通证据缺口、共享 hard fail、单来源与多来源原生分差、三类机会状态、反方向 watch 和共享触发行为测试；旧生产代码出现10个预期失败，进一步收紧共享触发与当前确认边界时再出现2个预期失败。修复后定向21项、相关主线103项、PM/PG保护108项、Phase-flow 312项和全量809项全部通过，`compileall src`、`git diff --check` 及 2025-03-26至03-27正式无 LLM 回测前门禁通过。生产 `--reset-config` 真实重跑因外部 LLM/PandaAI 数据传输安全审批拒绝而未启动，数据库未重置，新的 Phase1-Phase4、Researcher 和每日 PG 未登记为通过。

==========2026年7月17日==========

（1）[LLM 路由配置同源] `pg_preflight.py` 改为调用运行时 `llm_audit_metadata` 解析所选 Provider 的默认 URL 和密钥环境变量名，不再强制 `dev.yaml` 重复登记；`dev.yaml` 将 GPT 与 Claude 写成可整块注释切换的独立配置，GPT 仅保留专属 `reasoning_effort`，Claude 直接使用 `Provider.TQXAI` 内置路由。新增行为测试在旧实现先出现 `llm_api_key_env_missing/llm_base_url_missing`，修复后 GPT 与 Claude 默认路由均通过且不实例化 LLM；配置/入口定向10项、编译检查及正式无 LLM 回测前门禁通过。原因：消除 Provider 与主配置的 URL、密钥变量名和空 fallback 重复事实，同时保持密钥只从 `.env` 读取并维持单一 LLM 路由语义。

（2）[DeepSeek v4 Pro 主路由接入] `dev.yaml` 新增可整块注释切换的 `DeepSeek/deepseek-v4-pro` 配置，启用 `thinking.type=enabled` 与 `reasoning_effort=high`，继续由 `Provider.DEEPSEEK` 唯一提供 `https://api.deepseek.com` 和 `DEEPSEEK_API_KEY`；`pg_preflight.py` 将 DeepSeek 纳入无模型调用的正式路由就绪检查。三个分析师与 Researcher 继续共同消费顶层 `llm`，实际 provider/model 只落既有 config 运行元数据和非敏感路由审计，不进入 AEC、SCC、分析师 artifact 或 Researcher 学习 payload。新增行为测试在旧实现先因 `llm_provider_not_supported_by_current_config` 失败，修复后配置/分析师定向14项、信息隔离/Researcher/冻结主线137项及全量811项通过，`compileall src` 和 2025-03-26 正式无 LLM 回测前门禁通过；使用一次性 Workshop 探针经统一 `agent_call` 实际调用 DeepSeek 成功，探针随后删除，未输出密钥、prompt、原始 response 或推理内容。原因：在不复制 URL/密钥配置、不新增智能体私有路由和不泄露内部信息的前提下，增加可验证的 DeepSeek 高思考模型选择。
