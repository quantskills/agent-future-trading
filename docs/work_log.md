...
（5）补齐分阶段 artifact 保存边界，修正 Phase2 镜像 PM 解释字段的非策略污染。
修改了什么：`src/agents/execution_team/trader.py`、`src/util/futures_audit.py`、`src/tools/agent_tools/control/system_invariants.py`、`src/tools/agent_tools/control/contract_coverage_audit.py`、`src/tests/test_phase_flow_regression.py`、`src/tests/test_system_invariant_audit.py`、`docs/mechanism_multiagents.md`、`docs/unified_field_semantics.md`。
为什么改：2025-03-03 回测前验收暴露 Phase2 artifact 把 PM 的完整 `final_action_contract` 镜像进 `pm_plan_validation/setup_execution_learning`，导致 `position_sizing_result.capital_allocation_reason` 这类 PM 资金解释字段和执行意图字段出现在同一执行 payload，被系统不变量正确判定为 `opportunity_ranking_field_used_in_execution_trade_intent`。本次不新增交易合约、不新增交易字段、不改变 PM 唯一合约；完整 `final_action_contract` 仍只作为 PM recommendation artifact 的策略交易事实保存，交易员仍读取审计通过后的合约执行，但 Phase2 artifact 和 transaction audit payload 只能保存执行事实、触发事实、执行结果、执行学习 trace 和执行必要字段摘要，不能复制 PM 学习、排名、资金部署解释。新增 `artifact_phase_boundary` 契约覆盖和系统不变量测试，锁住 PM artifact 可保存完整合约、Phase2/transaction artifact 不得保存 PM 解释字段与交易意图混用。
