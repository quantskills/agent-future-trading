...
| `trigger_confirmation_adjustment` | `entry_quality_outcome` / `final_action_contract` / Trader 执行摘要 | 结构化入场确认调整。只允许既有 `not_applicable/neutral/standard_confirmation_supported/stronger_confirmation_required/strict_confirmation_required`；正式开仓 episode 的生成只按手续费后 `return_on_notional` 判断，盈利生成standard、亏损生成stronger、亏损达到2%名义收益率满惩罚边界时生成strict，禁止人民币固定金额决定等级。weak-conflict权限或同品种、同方向、同setup、同canonical trigger的正式canonical open/add episode学习可要求更强确认。Trader对stronger/strict `breakout/pullback/vwap_confirmed`等待下一根完整15分钟线保持在对应边界/VWAP正确侧，并要求触发线与延续线平均成交量不低于此前最多4根完整信号线的平均量（没有可比历史线时，两根确认线成交量均须大于0），再于下一合法1分钟成交；standard不进入该额外量能门。不得解析reason文本或改变方向、手数、rank。 |
