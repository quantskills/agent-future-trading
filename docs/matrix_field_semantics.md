...
| `opening_authority_type` | `final_action_contract` / Trader 生命周期摘要 | 当前持仓周期原开仓 FAC 的权限身份。首次新增风险时等于该开仓 FAC 的 `authority_type`；后续 hold/reduce/exit 必须沿原开仓 FAC 传递，用于区分 `exploration_probe` 与 real/scale 的既有持仓期限规则，不创建新动作、手数或退出权限。 |
...
| `position_lifecycle_trace.position_pnl_ratio` / `cycle_net_pnl` | PM 持仓生命周期诊断 | 从原开仓 FAC 起累计已结算逐日盈亏减手续费，再加当前决策时点单日未实现盈亏形成的完整周期净收益及既有保证金比率；盈利保持、普通亏损复核、趋势判断和 probe 生命周期读取该口径。 |
| `position_lifecycle_trace.cycle_peak_net_pnl` / `cycle_profit_drawdown` | PM 持仓生命周期诊断 | 原开仓 FAC 周期内手续费后累计净收益峰值及当前相对峰值的利润回吐额；只供 PM 次日唯一 hold/reduce/exit FAC 决策和复盘，不产生 Trader 盘中退出。 |
...
| `exploratory_hypothesis.payload_json.support_episode_ids` / `setup_type` | Researcher 探索研究 | LLM 假设必须引用本次实际提供且与 ticker/sector/side/horizon/regime/setup 作用域匹配的完整 episode ID；无有效支持 ID 的输出拒绝落库。 |
| `exploratory_hypothesis.status` / `payload_json.research_validation` | Researcher 探索研究 | 状态固定为 `candidate/monitoring/validated/rejected`。只使用假设生成日之后、同完整作用域真实 episode 的手续费后 `return_on_notional`；样本达标且均值为正才为 `validated`，最新亏损、均值非正或到期样本不足为 `rejected`。candidate/monitoring 不进入分析师提示词，验证过程不写 Profile、action-value、policy、Rank、仓位或 Trader 权限。 |
