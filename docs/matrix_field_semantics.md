...
| `trigger_confirmation_adjustment` | `final_action_contract` / Trader 执行摘要 | 结构化入场确认调整。只允许既有 `not_applicable/neutral/standard_confirmation_supported/stronger_confirmation_required/strict_confirmation_required`；weak-conflict权限或同品种、同方向、同setup、同canonical trigger的正式canonical open/add episode学习可要求更强确认。Trader对stronger/strict `breakout/pullback/vwap_confirmed`等待下一根完整15分钟线保持在对应边界/VWAP正确侧，再于下一合法1分钟成交；不得解析reason文本或改变方向、手数、rank。 |
