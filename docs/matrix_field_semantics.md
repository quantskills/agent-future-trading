...
本文是 agent-future-trading 的字段语义矩阵。系统从分析、决策、审计、执行、结算、复盘、研究、学习到评估，只允许使用本文定义的字段语义。
...
| `contract_code` | 合约 / 成交 / 结算 | 具体期货合约；agent-future-trading 业务层唯一格式为大写品种代码加四位 `YYMM`，不带交易所后缀。 |
| `FuturesDailyQuote.contract_id` / `FuturesDailyQuoteOptimized.ticker` / `FuturesMargin.contract_id` / `get_main_contract_code()` / `get_china_futures_contracts()` / `get_futures_minute_bars()[].trading_code` | PandaAI 对外业务出口 | 与 `contract_code` 使用同一具体合约身份。PandaAI 生产端按具体 `symbol`、主连 `dominant_id`、`trading_code` 的固定顺序生成；郑商所三位年月只按行情记录 `trade_date` 展开，禁止使用当前系统日期，禁止输出 `_DOMINANT`。分钟出口只在返回副本覆盖 `trading_code`，保留 PandaAI 原始 `symbol`、`dominant_id`、`exchange`。 |
...
| `rank_score_policy` | config catalog / `src/config/rank_score_policy.yaml` / runtime config | 唯一全市场资金 rank 的评分配置。`rank_score` 下的参数组固定与八个 `rank_score_components` 同名：`cold_start_evidence_quality`、`capital_layer_priority`、`open_add_action_value_delta`、`product_setup_trigger_history`、`trigger_execution_quality`、`calibrated_forecast_value`、`capital_efficiency`、`conflict_risk_invalidation_penalty`；嵌套权重键与 Python 实际消费的输入字段同名。它不创建交易权限、不改变资金层；探索层只允许同一个最终 `rank_score` 在 0.008 至 0.015 内确定计划比例，不覆盖 20% 总保证金或 0.5 净敞口红线。40 个干净交易日后才允许依据 rank 分层平均收益微调。 |
...
| `forward_forecasts` | 分析师 AEC / SCC / Researcher | 固定覆盖 1、3、5、10 个交易日的多期限预测列表。每个期限只保存方向概率、预期收益及区间、主要驱动和预测失效条件；不含手数、资金 Rank、交易动作和执行权限。 |
| `horizon_days` | `forward_forecasts` / `analyst_forecast_evaluation` | 预测及到期评价的交易日窗口，只允许 1、3、5、10。 |
| `up_probability` / `down_probability` / `range_probability` | `forward_forecasts` | 对应期限上涨、下跌、震荡的概率分布，三者均在 `[0,1]` 且合计为 1。它们是待未来样本校准的预测，不等于 LLM 原始 `confidence`。 |
...
| `adaptive_policy_applied` | `final_action_contract.learning_used` | PM Step6 对当日实际生效 adaptive policy 的唯一正式列表。每条只允许 `id/policy_type/policy_action/ticker/side/setup_type/horizon_class/market_regime/source_trading_date/valid_until`；仅记录真实改变评分、技术参数、仓位比例或资金层的 policy，单纯检索、匹配但未改变结果、被后续规则覆盖的 policy 不得写入。Researcher 只从该字段生成 `research_position_feedback.policy_refs_json`，它不创建交易权限，也不替代 action-value 归因。 |
...
| `opening_authority_type` | `final_action_contract` / Trader 生命周期摘要 | 当前持仓周期原开仓 FAC 的权限身份。首次新增风险时等于该开仓 FAC 的 `authority_type`；后续 hold/reduce/exit 必须沿原开仓 FAC 传递，用于既有持仓期限处理。它不改变盈利完全回吐的统一减仓复核口径，也不创建 Trader 动作、手数或盘中退出权限。 |
...
| `trigger_confirmation_adjustment` | `entry_quality_outcome` / `final_action_contract` / Trader 执行摘要 | 结构化入场确认调整。只允许既有 `not_applicable/neutral/standard_confirmation_supported/stronger_confirmation_required/strict_confirmation_required`；正式开仓 episode 的生成只按手续费后 `return_on_notional` 判断，盈利生成standard、亏损生成stronger、亏损达到2%名义收益率满惩罚边界时生成strict，禁止人民币固定金额决定等级。weak-conflict权限或同品种、同方向、同setup、同canonical trigger的正式canonical open/add episode学习可要求更强确认。Trader对stronger等待初次触发后1根完整15分钟线，对strict等待连续2根完整15分钟线；每根必须保持在对应边界/VWAP正确侧，且初次触发线与全部延续线的平均成交量不低于此前最多4根完整信号线的平均量（没有可比历史线时，整段确认线成交量均须大于0），再于下一合法1分钟成交。standard不进入该额外量能门；不得解析reason文本或改变方向、手数、rank。 |
...
| `execution_contract` | Trader Phase2 执行摘要 / 执行 payload | 从已审计 `final_action_contract` 白名单抽取的执行摘要，不是第二张交易合约。只能包含 `execution_profile`、`trigger_source`、`trigger_confirmation_adjustment`、`entry_trigger`、`invalidation`、`invalidation_level`、`valid_until`、`requires_intraday_confirmation`、`can_execute_without_intraday_trigger`、`authority_type`、`opening_authority_type`、`max_allowed_margin_ratio`、执行相关 `reason_codes` 和 `execution_action_value_preference`；不得包含完整AEC、目标手数、rank、资金或 PM 学习解释。`opening_authority_type`只供既有持仓生命周期规则识别原开仓权限，不是第二个交易权限。 |
...
| `position_lifecycle_trace.daily_position_pnl_ratio` | PM 持仓生命周期诊断 | 当日价格相对上一结算基准形成的单日浮动收益率，只供既有单日硬风险使用，不得冒充原开仓 FAC 完整周期收益。 |
| `position_lifecycle_trace.position_pnl_ratio` | PM 持仓生命周期诊断 | 生命周期统一收益率入口。有原开仓上下文时优先等于完整周期手续费后 `cycle_return_on_notional`；缺少该上下文时才回退为当前未实现盈亏除以当前保证金。普通持仓-2%减仓/-4%退出复核、新仓-0.5%减仓/-2%退出复核、盈利保持、趋势判断和probe期限规则读取该字段；不得附加“周期峰值未为正”条件。 |
| `position_lifecycle_trace.cycle_net_pnl` / `cycle_open_notional` / `cycle_return_on_notional` | PM 持仓生命周期诊断 | 从原开仓 FAC 起累计已结算逐日盈亏减手续费，再加当前决策时点未实现盈亏形成完整周期净收益，并除以原开仓名义金额得到手续费后完整周期收益率。该口径不使用会随减仓变化的当前保证金。 |
| `position_lifecycle_trace.cycle_peak_net_pnl` / `cycle_peak_return_on_notional` / `cycle_profit_drawdown` / `cycle_profit_drawdown_on_notional` | PM 持仓生命周期诊断 | 原开仓 FAC 周期内手续费后累计净收益峰值、峰值收益率及当前相对峰值的利润回吐。峰值为正、当前 `cycle_return_on_notional<=0` 且同向证据复核失败时，所有仓位类型均由 PM 走既有减仓复核；不产生 Trader 盘中退出。 |
...
| `current_evidence_quality` | PM scorecard / `direction_evidence_components` | 只汇总当日方向、可交易状态、业务/setup质量、置信度、市场确认、融合一致性及当日冲突/缺口扣分的当前证据质量，不含 action-value、Profile 或尾损历史；用于证明新增风险先满足现有当前证据门槛。 |
| `validated_learning_delta` | PM scorecard / `direction_evidence_components` | 已通过现有时效、作用域和正式学习过滤的正负 action-value、尾损、入场/触发质量及 Profile 净修正；只能在当前证据入场前提满足后参与 `opportunity_score`、Rank 和 0.8%～1.5% probe 差异化，不得单独把机会变成可交易候选。 |
| `current_entry_eligible` / `current_entry_prerequisite_met` | PM scorecard | 前者表示不存在缺方向、缺可交易状态、缺setup、缺失效边界、关键数据缺口或未解决主导反对证据；后者还要求当前证据达到现有 tradeable 门槛或现有单一可交易 setup 确认。任一为假时，历史学习不得生成 probe/real/scale。 |
...
| `candidate_quality` / `candidate_quality_components` | PM唯一scorecard / Step4资金层内比例 | `candidate_quality=clamp(opportunity_score + 0.04*trigger_valid + 0.04*invalidation_present, 0, 1)`；setup、正式学习/profile和冲突已经在`opportunity_score`中计入，不得由Step2或后置控制重复加入。它是层内比例质量，不是有符号rank。 |
| `positive_learning_signal` / `negative_learning_signal` | `action_value_learning_summary` / `rank_score_policy.rank_score.open_add_action_value_delta` | canonical open/add 正向与负向学习的归一化聚合强度，均只读取手续费后完整周期 `return_on_notional`；同完整作用域最新周期为亏损时，正向信号立即清零，负向信号至少保留该最新亏损强度。配置中的同名参数是各信号进入 `open_add_action_value_delta` 的乘数；禁止使用 `positive_signal`、`negative_signal` 或字符串前缀推断作为别名。 |
...
| `alpha_profile_adjustment` | `opportunity_score_components` / `rank_score_policy.rank_score.product_setup_trigger_history` | 产品/setup/trigger 历史 profile 对候选质量的净修正；`candidate/watchlist` 不得产生正数，`protected/deployable` 也必须同时存在未被最新亏损撤销的正式正向 action-value 才可加分，`capped/rejected` 保留负向作用。配置中的同名参数是其进入 `product_setup_trigger_history` 的乘数。 |
...
| `rank_score` | PM 第 5 步工具 `pm_full_market_capital_deployment` / `rank_score_policy` | 唯一全市场资金 rank 的直接排序分数。它是八个 `rank_score_components` 的有符号总和，不做 `[0,1]` 截断；可为负并保留大小关系。它只表达相对投资价值和资金优先级，不是盈利概率、盈利金额或交易禁入条件。权重来自 `src/config/rank_score_policy.yaml`，只能由 `pm_full_market_capital_deployment` 在新增风险候选池中生成。PM scorecard / `pm_signal_fusion` 只能提供输入组件。 |
| `rank_score_input_components` | PM Step4 `pm_signal_fusion` / PM Step5 `pm_full_market_capital_deployment` | Step4 交给 Step5 的 rank 原始输入对象；保存不含学习、profile 和风险重复项的 `cold_start_evidence_quality`，以及当日三分析师同期限概率分布经到期历史评价校准后的 `forecast_calibration`。它不是最终 `rank_input_components`，不得包含 `final_rank_score_generated_by` 或预生成 rank。 |
| `rank_score_components` | PM Step5 `pm_full_market_capital_deployment` / `rank_input_components` | `rank_score` 的确定性拆解，固定包含 `cold_start_evidence_quality`、`capital_layer_priority`、`open_add_action_value_delta`、`product_setup_trigger_history`、`trigger_execution_quality`、`calibrated_forecast_value`、`capital_efficiency`、`conflict_risk_invalidation_penalty`；字段名与 `rank_score_policy.rank_score` 的八个参数组一一对应。 |
...
| `calibrated_forecast_value` | `rank_score_components` / `rank_score_policy.rank_score` | 由三名分析师当日同一候选期限的方向概率和预期收益，经对应分析师、期限、信号侧的到期方向命中率、Brier、市场状态及预测方向手续费后收益校准后形成的有符号排序分项；中性分析师保留概率分布参与，冷启动固定为 0，负值只降低相对顺序，不禁止交易。 |
| `capital_layer_priority` / `open_add_action_value_delta` / `product_setup_trigger_history` / `trigger_execution_quality` / `capital_efficiency` / `conflict_risk_invalidation_penalty` | `rank_score_components` / `rank_score_policy.rank_score` | Step5 的其余六个 rank 分项；catalog 同名参数组分别控制候选层级积分、canonical open/add 学习、产品历史、触发质量、资金效率和冲突风险扣分。`capital_layer_priority` 只加减 rank 分数，不生成或改变 `capital_layer`、`final_entry_authority`。配置名、Python 读取名和最终 trace 名必须一致。 |
...
| `capital_deployment` | `final_action_contract` / PM 资金部署 / 复盘评估 | PM 资金部署结果对象，记录候选是否入选、原目标手数、部署后目标手数、部署手数变化、部署原因和新增风险排名；探索层同时记录唯一最终 Rank 对应的区间强度、计划保证金比例、手数取整后的实际保证金比例及目标手数。它只能解释并回写同一张 `final_action_contract`，不能作为第二交易权限。所有最终合约都必须原子写入该对象；只有实际增加风险的对象允许包含 `opportunity_rank`，包括新开仓和同方向扩大绝对手数的 `add/scale`。 |
...
| `entry_quality_outcome` | `product_learning_performance_key` / `alpha_setup_action_value.payload_json` | Researcher 把已结算开仓 episode 的手续费后 `return_on_notional` 绑定回原始 setup、entry_trigger、evidence_combo 和 deployment_tier 的未来学习归因；`learning_economics_basis`固定为`after_fee_return_on_notional`，正负、`support_weight/penalty_weight`及生成的`trigger_confirmation_adjustment`均使用该收益率，人民币`net_pnl`只保留审计事实。该结构用于下一轮分析师、PM入场质量、触发质量和资金优先级校准，不创建交易权限，不修改当天事实。 |
...
| `product_learning_calibration_view` | 分析师 `learning_context.alpha_setup_items` / `memory_trace` / `learning_impact_summary` | `product_learning_performance_key` 的分析师安全视图；只保留产品、方向、setup、trigger、证据组合、历史部署层级、历史 PM rank/score 别名、手续费后收益率和最新完整周期结果，用于校准证据质量与待验证问题。同完整作用域最新亏损时，正式action-value投影撤销该作用域旧正向Profile校准。该视图不得包含 `authority_type`、`final_action`、`target_lots`、`lots_delta`、`opportunity_rank`、`capital_allocation_reason` 等 PM 权限字段原名。 |
...
| `learning_used.alpha_setup_action_values` | `final_action_contract.learning_used` | PM 最终合约实际声明消费的正式 canonical action-value 主证据列表；必须且只能来自同一合约最终 `decision_learning_rows`，不得包含 execution/profile 行。行级纯净性和生命周期匹配口径由 `docs/agent_pm.md` 与 `docs/matrix_action_canonical.md` 固定。 |
...
| `reward_mean` | action-value | 平均人民币奖励；服务既有action-value生命周期、升层和财务审计，不进入开仓episode的分析师学习强度、PM Rank、仓位学习方向或触发确认等级。 |
| `mean_return_on_notional` | action-value payload / 分析师安全投影 | 同一正式作用域内完整策略 episode 的平均手续费后名义收益率；分析师开仓学习强度、PM Rank及仓位学习的正负和强度均使用该收益率，不使用人民币奖励大小。人民币奖励继续服务既有 action-value 生命周期分类、升层门槛和审计。 |
...
| `latest_complete_episode_return_on_notional` / `latest_complete_episode_date` / `latest_complete_episode_outcome` | action-value payload / 分析师安全投影 / PM 正式学习摘要 | 同完整作用域最新真实完整周期的手续费后名义收益率、结算完成日及 `profit/loss/flat` 结果；用于让最新亏损优先撤销分析师与PM旧正向学习，不按人民币盈亏比较，也不单独禁止当日强证据试仓。 |
| `learning_economics_basis` | `signal_calibration` / `entry_quality_outcome` / 分析师安全投影 | 开仓episode学习经济口径，固定为`after_fee_return_on_notional`；声明分析师证据校准和触发确认生成只读取手续费后名义收益率。非开仓生命周期校准记录为`lifecycle_reward`，不得混入开仓收益率强度。 |
| `latest_complete_episode_loss` / `positive_amplification_suspended` | `signal_calibration` / 分析师学习校准 / `action_value_learning_summary` / `learning_adjustment_summary` / PM alpha 学习融合 | 同完整作用域最新完整周期亏损及旧正向放大暂停的机器事实。为真时，分析师旧正向Profile校准、PM旧`positive_learning`、正向Profile加分、positive open seed、real/scale放大资格全部失效；当日有效证据仍可沿既有exploration probe路径验证，不创建新权限。 |
| `recent_complete_episode_count` / `recent_mean_return_on_notional` / `recent_negative_expectancy` | `alpha_setup_profile.payload_json.stats/classification` / action-value `profile_lifecycle` | 同完整作用域最近最多5个真实完整周期的手续费后名义收益率统计及是否转为负期望。样本达到既有 `cap_min_samples` 且均值小于0时复用 `capped`；后续 probe 使滚动均值恢复非负后，继续由既有 watchlist/protected/deployable 门槛决定恢复，不新增 lifecycle 状态。 |
...
| `rank_input_components.rank_score_components.cold_start_evidence_quality` / `capital_layer_priority` / `open_add_action_value_delta` / `product_setup_trigger_history` / `trigger_execution_quality` / `calibrated_forecast_value` / `capital_efficiency` / `conflict_risk_invalidation_penalty` | PM rank 分项 | 唯一 rank 的八项确定性评分；配置名与 Python 消费字段一一对应。 |
...
| `capital_deployment.rank_budget_sequence` / `rank_score` / `candidate_margin_ratio` / `queue_margin_ratio_before` / `queue_margin_ratio_after_if_selected` / `target_margin_ratio_budget` | PM Step5 预算队列；Phase2 strategy recommendation scheduler 只读消费 | `rank_budget_sequence` 是 PM 已签的新增风险资金消费顺序。Phase2 先让不增加风险的真实 `reduce/exit` 按既有稳定顺序处理，再只对具有严格正整数 sequence 的新增风险推荐在其余稳定槽位间升序调度；缺失或非法值及wait/hold留在原槽位。当前生产PM将异号目标收敛为exit-only，不得借调度器或Trader防御性反转翻译取得反向新增风险权限。不得读取 `opportunity_rank/rank_score/learning` 重新排序，不得生成或修改 rank，也不得据此取得交易权限或修改 FAC。其余字段继续表示分数、候选保证金和队列前后保证金。 |
| `capital_deployment.rank_scaled_exploration_strength` / `rank_scaled_exploration_planned_margin_ratio` / `rank_scaled_exploration_actual_margin_ratio` / `rank_scaled_exploration_target_lots` / `rank_scaled_exploration_min_margin_ratio` / `rank_scaled_exploration_max_margin_ratio` / `rank_scaled_exploration_lot_rounding` | PM Step5 探索层资金部署 | 仅 `exploration_probe` 写入：最终 `rank_score` 的 `[0,1]` 有界强度、0.008 至 0.015 内唯一计划比例、按既有合约最小手数取整后的实际保证金比例和目标手数。非正 Rank 保留 0.008 计划下限；字段不改变资金层、交易权限、Trader 触发和退出链。 |
...
| `phase2_execution.current_lots_before` / `two_step_reversal` | Trader Phase2 | 执行前持仓及防御性订单翻译是否检测到异号超额平仓。当前生产策略PM已将反向目标原子收敛为`exit/target_lots=0`，该字段不得作为同一recommendation反向开仓权限。 |
...
| `src/config/dev.yaml::max_total_margin_ratio` | `src/tools/agent_tools/control/pg_pre_backtest_acceptance.py::_config_mapping_check`；`src/tools/agent_tools/execution/trader_futures_execution.py::FuturesExecutionEngine._build_transaction` | 账户总保证金硬上限；同一顶层参数供回测前配置映射校验和策略新增风险在动态保证金确定后的最终成交硬线复核。 |
...
| `src/config/dev.yaml::execution.dynamic_margin.**` | `src/tools/agent_tools/execution/trader_futures_execution.py::_resolve_dynamic_margin_rate` | 动态保证金来源及静态缓存回退；最终解析结果供同一执行引擎的成交前账户硬线复核。 |
...
| `src/config/portfolio_policy_catalog.yaml::ticker_loss_control.**` | `src/agents/decision_team/portfolio_manager.py::_apply_drawdown_and_ticker_loss_control` | 品种连续亏损的同作用域软风险标签；不得在Step4最终计划后以名义仓位比例二次改写保证金计划。旧`ticker_performance_control`已删除。 |
...
| `src/config/rank_score_policy.yaml::rank_score_policy.rank_score.**` | `src/tools/agent_tools/decision/pm_full_market_capital_deployment.py::_rank_score_components_for_row` | Step5 唯一 rank 的八个分项权重、学习修正边界、资金效率和风险扣分参数；不改变交易权限与资金层，探索层只在既有0.008至0.015区间内按最终Rank分配。 |
