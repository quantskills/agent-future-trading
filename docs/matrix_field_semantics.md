...
共享解释器：`src/tools/common/final_action_semantics.py` 是全系统唯一的确定性交易语义状态机。它不调用 LLM，不签合约，不下单，不入账，不写研究；只统一解释分析师证据禁用字段、信号收集边界、`final_action_contract` 全生命周期、`reason_codes` 分类、条件监控、直接执行、普通持有、硬阻断、软降级、未触发、已触发成交、扩大交易、减仓、退出，以及 action-value 的 `action_name -> canonical_action_family -> action_value_lane/learning_lane -> action_preference`。动作 canonical 矩阵见 `docs/matrix_action_canonical.md`；可执行口径以 `final_action_semantics.py` 为准。Protocol Governor 只能通过该工具解释学习 family/lane 匹配、`final_action + current_lots + target_lots + lots_delta` 一致性、no-change / rank / learning 无仓位变化解释、active opportunity rejection 和 open transaction blocker；PG 不得保留私有 reason code 词表、私有 action-value 动作集合和私有 final_action 推断口径。
...
| `signal_record_id` | `signal_collection_contract.source_contracts` | 分析师 `signal` 表记录 ID，仅用于 Reviewer、Researcher 和 PG 来源追溯，不创建交易权限。 |
...
| `opportunity_rank` | PM 全市场资金部署工具 `pm_full_market_capital_deployment` / `final_action_contract.evidence_used` / `capital_deployment` / 复盘评估 | 当日所有 `current_lots=0` 且 `target_lots!=0` 的开仓候选进入同一个全市场资金候选池后的唯一资金优先级排序；`rank=1` 固定表示当天全市场最值得占用开仓资金的产品机会。它可以对应小探、正常真实资金或学习验证后的放大资金，但不生成第二张合约。PM scorecard 的单品种方向排序不得写入该字段；`add/scale/hold/reduce/exit/reverse` 当前合约不得生成该字段，反转只有在旧方向退出后形成新的空仓开仓合约时才重新排名。 |
...
| `primary_lifecycle_action_port` | `portfolio_manager.py` 第 3 步 / PM 内部学习与诊断 trace | PM 主链第 3 步生命周期分流口，必须在第 2 步方向选择后、学习路由前由 `pm_lifecycle_action_port.py` 生成。它只服务 PM 内部分流、学习路由和 provenance；不得写入 `final_action_contract.evidence_used`，也不得作为 Step6 最终合约失败依据。Step6 是否需要 Step5 只能按第 4 步最终候选是否满足 `current_lots=0` 且 `target_lots!=0` 判定；`add/scale` 的生命周期仍属 `open_add_new_risk` 学习 family，但 `requires_full_market_rank=false`。 |
...
| `pm_six_step_trace.step6_contract_generation_check` | `signal_snapshot.pm_six_step_trace` | Step6 签约时生成的最终合约生成合法性检查。它只检查最终 `final_action_contract` 是否由合法 PM 机制生成，包括手数动作自洽、从空仓建立新仓是否具备 Step5 deployment、非开仓合约不伪造 rank、Step5 未部署是否还原为空仓且无盘中开仓权限、`capital_deployment` 语义完整、PM 中间态不得进入保存 artifact。它不比较 Step2 与 Step6 是否一致。 |
...
| `capital_deployment` | `final_action_contract` / PM 资金部署 / 复盘评估 | PM 资金部署结果对象，记录候选是否入选、原目标手数、部署后目标手数、部署手数变化、部署原因和开仓排名；只能解释并回写同一张 `final_action_contract`，不能作为第二交易权限。所有最终合约都必须原子写入该对象；只有从空仓建立新仓的对象允许包含 `opportunity_rank`。 |
| `no_rank_no_new_exposure` | `final_action_contract.reason_codes` / `capital_deployment.capital_allocation_reason` | 从空仓建立新仓的候选未获得 PM 全市场开仓 rank 时的确定性还原原因；最终合约必须把 `target_lots` 还原为 `current_lots=0`，不得由 workflow fallback 保留 `open/open_probe/open_real/conditional open` 的目标手数。该既有原因码不适用于 `add/scale`。 |
| `no_rank_or_budget_no_new_exposure` | `final_action_contract.reason_codes` / `capital_deployment.capital_allocation_reason` | 开仓候选已经进入全市场 rank 队列，但按 rank 顺序消耗总保证金、单品种或净敞口预算时未被选中；最终合约必须还原为 `wait` 且 `target_lots=current_lots=0`，不能绕过预算继续开仓。 |
| `contract_requires_conditional_intraday_result` | `final_action_semantics` / PG 机制审计 / Trader 盘中结果要求 | 条件候选是否需要 Trader 写盘中触发/未触发结果的统一只读语义。只有最终合约仍然部署非零目标仓位的条件开仓才需要盘中结果；已经被 `no_rank_no_new_exposure` 或明确未选中原因还原为 `wait`、`target_lots=current_lots=0`、`selected_for_capital_deployment=false` 的未部署条件候选，不要求 `opportunity_rank`，也不要求 Trader 写盘中结果。`add/scale/reverse` 当前合约不经过全市场开仓 rank。 |
