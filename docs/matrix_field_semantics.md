...
| `opening_authority_type` | `final_action_contract` / Trader 生命周期摘要 | 当前持仓周期原开仓 FAC 的权限身份。首次新增风险时等于该开仓 FAC 的 `authority_type`；后续 hold/reduce/exit 必须沿原开仓 FAC 传递，用于区分 `exploration_probe` 与 real/scale 的既有持仓期限和盈利完全回吐处理。它只选择 PM 既有生命周期分支，不创建 Trader 动作、手数或盘中退出权限。 |
...
| `position_lifecycle_trace.cycle_peak_net_pnl` / `cycle_peak_return_on_notional` / `cycle_profit_drawdown` / `cycle_profit_drawdown_on_notional` | PM 持仓生命周期诊断 | 原开仓 FAC 周期内手续费后累计净收益峰值、峰值收益率及当前相对峰值的利润回吐。峰值为正、当前 `cycle_return_on_notional<=0` 且同向证据复核失败时，探索仓由 PM 签唯一exit，real/scale保持既有减仓复核；不产生 Trader 盘中退出。 |
...
| `rank_score` | PM 第 5 步工具 `pm_full_market_capital_deployment` / `rank_score_policy` | 唯一全市场资金 rank 的直接排序分数。它是七个 `rank_score_components` 的有符号总和，不做 `[0,1]` 截断；可为负并保留大小关系。它只表达相对投资价值和资金优先级，不是盈利概率、盈利金额或交易禁入条件。权重来自 `src/config/rank_score_policy.yaml`，只能由 `pm_full_market_capital_deployment` 在新增风险候选池中生成。PM scorecard / `pm_signal_fusion` 只能提供输入组件。 |
