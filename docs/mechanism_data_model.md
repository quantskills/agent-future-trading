...
| `decision_memory_retrieval` | 否 | 确定性读取结构化研究记录 | 自由文本记忆解释、手数、交易动作 |
...
可受限调用 LLM。通过 `run/research/researcher_learning.py` 在复盘员 Phase4 验证完成后单独运行，基于复盘事实、完整 episode、未交易机会、未触发条件机会和执行结果，写入结构化研究成果，包括：
...
研究记录必须结构化写入。自由文本只能解释，不是下游直接消费的研究结论。
...
- `signal_collector` 只读取已被分析师校准后的结构化信号；
- `auditor` 只审合约里的 `learning_used` 和资金理由；
- `trader` 只执行已吸收研究影响后的合约和合约化触发规则；
- `accountant` 只结算真实成交；
- `reviewer` 只复盘合约、执行和结算结果；
- `protocol_governor` 只审计研究记录是否正确传递。
