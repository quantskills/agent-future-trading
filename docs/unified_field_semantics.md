...
| `lifecycle_learning_trace` | `final_action_contract.evidence_used` / `capital_deployment` | rank 使用的生命周期学习路由轨迹。新资金 rank 固定为 `open_add_new_risk`，只允许 open/add/scale/increase 学习直接影响排序；hold/reduce/exit/execution/conditional_monitor 必须记录为 blocked/ignored，不能冒充开仓学习。 |
| `learning_impact_delta` | `final_action_contract.evidence_used` / `capital_deployment` | 学习对本次资金 rank 的净影响拆解，记录正向 open/add 学习、负向 open/add 学习、入场质量亏损、触发质量正负反馈等分项；`execution_profile_learning_direct_to_rank` 必须为 false。 |
