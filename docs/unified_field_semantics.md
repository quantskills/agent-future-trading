...
| `pm_six_step_trace.pm_contract_self_check` | `signal_snapshot.pm_six_step_trace` | `pm_contract_self_check.py` 对最终 `final_action_contract` 自身做机制边界检查的结果。它检查基础字段、手数动作一致性、rank/非 rank/Step5 未部署边界、`capital_deployment` 和 `position_sizing_result` 语义完整、无空对象冒充事实、无盘中触发权限残留、无 PM 中间态污染。workflow 和 PG 只读取该结果，不修合同。 |
