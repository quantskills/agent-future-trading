...
不足一手时不得为了“必须交易”而向上取整。当前反向目标只签`exit/target_lots=0`并释放原持仓；仓位归零后的后续反向新风险必须在新FAC中重新计算预算，不得在同一原子决策中把平旧仓与反向开新仓合并计算。
