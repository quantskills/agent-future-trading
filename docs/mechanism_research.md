...
| `no_trade_opportunity_memory` | 未交易机会、no-trade 原因、多周期影子结果、错过机会；方向、setup、入场触发、周期和市场状态直接继承对应 `final_action_contract` | 研究员汇总；分析师读取校准摘要；投资组合经理经 `decision_memory_retrieval` 间接消费 | 不能直接授权开仓，只能作为先验、反证或排序诊断；不得从分析师顺序、信号组合或推荐动作重建学习身份，FAC 身份不完整时不写该记录；fast-candidate 只按固定5日窗口统计同作用域完整正负样本，并要求未发生入场前失效且 FAC 具备canonical触发、profile、来源和失效边界，不把影子记录改造成 Trader 成交 |
