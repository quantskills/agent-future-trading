...
本文是 agent-future-trading 全链路契约矩阵。它只回答一件事：每个关键系统事实由谁生产、落在哪里、谁消费、谁审计、什么条件必须 hard fail、什么条件只进入 diagnostics。
...
|---|---|---|---|---|---|---|---|---|
| `action_evidence_contract` | `technical` / `fundamental` / `commodity_news`，Phase1 | 截止点内行情、基本面、新闻、商品 profile、分析师校准摘要 | 三份经同一共享校验的结构化预测证据，固定携带 1/3/5/10 日 `forward_forecasts` 概率分布、预期收益区间、驱动及预测失效条件。technical 固定 `entry_timing`，可执行 profile 只允许 `breakout/pullback/vwap_confirmed`；commodity_news 固定 `event_catalyst`，即时事件只允许 `event_immediate`；fundamental 固定 `direction_context` 且不得生产 Trader profile、entry invalidation。多期限预测不含手数、Rank和交易权限。technical 使用开仓前已完成OHLC确定性生成原始ATR14并由finalization写入`atr_stop_distance`，LLM不得生产或改写ATR。 | Workflow 在一个 Phase1 写事务中保存三份 signal 及全部 recommendation；内存metadata只保留同一AEC与真实ID | `signal_collector`、Reviewer、Researcher | 角色结构校验；shared finalization；共享 AEC 校验；pre-backtest structured IO | 含手数、rank或最终动作；多期限概率不归一、期限不全、收益区间倒置；entry trigger/profile/side/condition/level不一致；LLM生产ATR；部分落地 | 无方向、缺具体触发或入场作废边界、仅有研究价值、证据弱/冲突、合法普通中性或数据不可用状态 |
...
| `learning_used.adaptive_policy_applied` | 技术分析师已应用校准经 AEC→SCC 传递；PM policy 控制经 Step4/Step5 诊断传递；PM Step6 统一装配 | 当日实际改变技术参数、评分、仓位比例或资金层的 adaptive policy | 每条仅保存 `id/policy_type/policy_action/ticker/side/setup_type/horizon_class/market_regime/source_trading_date/valid_until` | `final_action_contract.learning_used.adaptive_policy_applied` | Reviewer、Researcher；`research_position_feedback.policy_refs_json` | 技术参数应用保真、实际 policy 收集、反馈独立归因回归；pre-backtest PM 输出契约测试 | 把单纯检索、未改变结果或被后续规则覆盖的 policy 写成已应用；缺 ID、动作、作用域或日期；依赖 action-value memory ref 才保留 policy ref；把 policy 直接当交易权限 | 当日没有合格或实际生效 policy 时列表为空 |
...
| `opportunity_scorecard` | PM 内部状态判断，Step2–4 | SCC、证据融合、当前持仓、风险上下文、冻结 canonical 学习池 | Step2 固定 `preferred_side/side_priority`；Step4 在学习消费完成后形成唯一最终scorecard，并决定生命周期、candidate quality、probe/real/alpha_scale；real/scale层内比例继续由candidate quality连续确定，probe只给出0.8%下限与1.5%上限并等待Step5唯一rank确定最终比例 | 同一个 PM 内存状态；Step6 仅写矩阵登记摘要 | PM Step4/Step5、Reviewer、Researcher | contract coverage；PM最终合约间接校验 | 学习重选方向；Step4读取未来rank；scorecard独立artifact或第二套交易计划；similar/weak/incomplete参与正式升层 | 冲突、候选质量低、无学习的合法冷启动probe或未获预算 |
...
| `rank_capital_layer_contract` | PM full-market capital deployment，Step5 | Step4已定层且实际增加风险的候选、资金状态、当日三分析师同期限预测、到期预测校准和rank政策 | 八项有符号 `rank_score` 只求和一次：冷启动证据 + 层级6/3/0 + canonical open/add历史学习 + setup历史 + 当日SCC触发质量×0.08 + 校准预测价值 + 一次资金效率 - 风险；校准项按候选期限精确消费1/3/5/10日概率分布，并用同分析师、期限、信号侧的到期方向准确率、Brier、市场状态和手续费后收益校准。只按score降序、ticker排序生成 `rank_budget_sequence`；探索层随后以该唯一score在0.8%至1.5%内确定计划比例，负分仍保留0.8% | `final_action_contract.capital_deployment` | PM self-check、Trader只读预算顺序、Reviewer、Researcher | 普通PM回归；pre-backtest PM路径 | 原始LLM confidence冒充盈利概率；跨期限或未到期预测进入Rank；排除中性分析师概率；第二排序；截断/负分禁入；非新增风险带rank | 合法负rank probe、预测冷启动、未获预算、同层证据或学习较弱 |
| `position_sizing_result` | PM position sizing tool | 持仓、合约乘数、保证金率、风险参数、目标资金层；探索层额外读取Step5最终rank确定的0.8%至1.5%计划比例 | 确定性手数计算结果 | PM 输入；`final_action_contract.evidence_used.position_sizing_result` | PM Step6、PM self-check、Auditor（仅取 `target_margin_ratio_estimate` 作为硬保证金上限输入）、Reviewer | PM self-check；contract coverage | sizing 工具直接签最终交易；空对象冒充 sizing；结果与 `target_lots/lots_delta` 不一致；Auditor 重算 sizing；探索层使用第二个rank或越过1.5%计划上限 | sizing 被风险上限或合约最小手数压缩 |
...
| `adaptive_policy_state` / `provisional_policy_state` / `config_learning_overlay` | Researcher / 配置学习写入工具，Phase4 后 | 长窗口研究结果、验证通过的参数证据、回滚值；成交结果政策继承其上游 FAC 身份；PM 新机会按当天 SCC/FAC 检索，已持仓、减仓、退出按原开仓 FAC 检索；PM 生命周期校准只读 FAC 中的 `pm_lifecycle_learning_impact_delta`；`technical_parameters` 使用独立 exact-ticker、short-horizon 技术绩效查询和独立配额；未交易机会只有固定5日、同作用域、完整正负样本且FAC可执行依据完整的 `missed_alpha_accountability` 路径可生成 `fast_candidate_alpha` | 策略参数学习状态、候选参数、回滚信息；市场状态统一使用小写下划线格式；技术参数 policy 继续写入同一 `adaptive_policy_state` 并由技术分析师沿原链消费；`fast_candidate_alpha`只允许下一交易日同作用域probe/小仓复核，Profile candidate/watchlist不生成该政策，未交易反事实不得生成成熟`alpha_promotion`；当前无合格参数优化生产者时不得复制原配置并标记为 overlay 刷新，既有原值复制 overlay 按原来源与原因精确停用 | research / config learning DB | 开发验收、技术分析师、PM 配置读取、PG | pre-backtest config consistency；技术参数独立配额与 exact-ticker 回归；PM 生命周期正式契约行为测试；data time boundary；新机会/持仓政策路由回归；fast-candidate生产来源、停用归属与消费权限回归 | technical policy 被 PM 共享配额挤出；跨品种通配 technical policy；未验证参数直接生效；持仓政策用当天反向机会身份检索；Profile与未交易路径共用fast-candidate生产权；未交易影子结果按最佳周期晋升成熟alpha；fast-candidate跨来源停用或错误来源进入PM；同义市场状态因空格/下划线格式不同降级；复制当前配置冒充已学习参数；从旧 PM 内部对象读取生命周期；缺 rollback；改当天交易事实；绕过 PM 合约权限；未来数据参与参数 | 候选策略待验证、样本不足、无合格参数优化结果时 overlay 写入为零 |
...
| `analyst_forecast_evaluation` / hierarchical performance | Researcher，Phase4 后 | 历史 AEC `forward_forecasts` 与 `data_usage_summary.trading_date`、预测日价格、到期结算交易日价格、执行手续费事实表 | 到期方向命中、Brier、标的实际收益、预测方向手续费后收益；再按品种、板块、策略/状态及全局层级形成 `analyst_performance` 和 `setup_type_performance` | research DB | 下一交易日分析师校准；PM Step4 读取摘要后仅交给 Step5 Rank | 固定期限、逻辑日期、future-only、分层样本回归 | 把 reference portfolio 的 Prev(T) 当预测日期；未到期评价；使用未来价格生成当日信号；校准摘要直接下单；精确作用域样本不足时整条学习链为空 | 预测冷启动、某层样本不足、到期价格缺失 |
| `research_position_feedback` | Researcher，Phase4 后 | action-value 从 `learning_used.alpha_setup_action_values` 与最终 `decision_learning_rows` 的正式交集归因；policy 只从同一 FAC 的 `learning_used.adaptive_policy_applied` 归因；再关联后续执行和结算事实 | `memory_refs_json` 与 `policy_refs_json` 分别记录实际消费的 action-value 和实际生效 policy，二者互不依赖 | research DB | Researcher 与开发评估诊断 | 普通反馈、policy-only 反馈、独立归因回归；daily PG 不复查内部学习作用过程 | 从 legacy selected refs、检索列表、未匹配记录或未生效 policy 伪造反馈；因 memory ref 为空而清空 policy ref；把反馈反向作为 rank、手数或交易输入 | 合法无学习、仅 action-value、仅 policy、学习被消费但未成交、两类引用均为空 |
