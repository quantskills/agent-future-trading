...
| `trade_episode_memory` / `no_trade_opportunity_memory` | Reviewer / Researcher，Phase4 后 | 已结算完整策略持仓周期及其物理pair经济明细、未交易机会、未触发条件机会 | 可为空的未来学习事实底座；episode 不是 formal action-value | research DB；artifact | Researcher；分析师与 PM 只消费后续按各自作用域形成的正式摘要/action-value，不直接读取 episode 表 | data time boundary；mechanism audit | 前视；未完成 Phase4/结算、裸 transaction、非策略、rollover 或 forced_risk 进入策略 episode 学习；要求每笔交易都形成学习 | 无合格 episode、学习为空、机会少、错过交易 |
