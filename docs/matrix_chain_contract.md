...
|---|---|---|---|---|---|---|---|---|
| `action_evidence_contract` | `technical` / `fundamental` / `commodity_news`，Phase1 | 截止点内行情、基本面、新闻、商品 profile、分析师校准摘要 | 三份经同一共享校验的结构化预测证据；三个 LLM 入口使用角色化输出模型，正常无方向、无具体触发、无 canonical 失效边界或仅有研究价值时允许 profile 为空并形成 `no_opportunity`；只有已声明且字段完整的 technical watch/probe/tradeable 或 news 即时候选漏填、错填 profile 时才进入既有有限 parse-error 重试。technical 固定 `evidence_role=entry_timing`，可执行时机只允许 `breakout/pullback/vwap_confirmed`；fundamental 固定 `direction_context` 且不得形成 Trader profile；commodity_news 固定 `event_catalyst`，只有当前事件已满足即时边界时才可形成 `event_immediate` 候选。可执行 `entry_trigger` 由共享 canonical 定义按时机和方向确定性生成；`risk_reduction_candidate` 只服务已有持仓；必需市场事实不可用时仍由三个分析师各自产生合法中性 AEC | Workflow 在一个 Phase1 写事务中保存三份 signal 及全部 recommendation；`artifact_json`和分析师报告都只保存同一AEC；内存metadata保存前仅AEC、保存后仅AEC与真实ID | `signal_collector`、Reviewer、Researcher | 角色化结构输出校验；shared finalization；共享 AEC 校验；pre-backtest structured IO 与同库普通中性/数据不可用中性预演；daily PG 只核对实际进入策略路径时物理来源记录存在 | 含手数、仓位比例、保证金、`final_action_contract`、`opportunity_rank`、最终交易动作；完整可执行候选缺失或使用非法 profile；角色/profile/trigger 不符合共享 canonical；fundamental 声明执行机会；news 声明普通15分钟 watch；自由执行文字进入正式 `entry_trigger`；Phase1失败后留下部分 signal/recommendation/artifact；伪造方向、profile、trigger、权限或市场事实 | 无方向、有方向但无具体触发、缺 canonical 失效边界、仅有研究价值、证据弱、证据冲突、基本面或新闻无新增、合法普通中性 `no_opportunity`、合法中性数据不可用状态 |
...
| `final_action_contract` | PM Step6，Phase1 | SCC、持仓、账户、Router 截止点内具体合约事实、配置、有效学习、PM 工具输出 | 唯一可执行策略合约：普通15分钟执行只选最终方向下合法 technical AEC，事件即时执行只选合法 commodity_news AEC；PM 先按执行职责过滤，再在同类合法证据中比较置信度。`execution_profile=entry_timing_signal`，`entry_trigger/invalidation/profile/source` 及失效价/ATR 来自同一证据；fundamental 只提供方向背景。执行 action-value 只保留为建议摘要，不改写顶层执行事实 | `futures_recommendation.signal_snapshot.final_action_contract`；recommendation artifact | Auditor、Trader、Reviewer、Researcher | PM self-check；Auditor；daily PG 只核对唯一交易来源及执行成交事实 | 新增风险缺合法具体合约；默认/猜测合约；执行字段跨分析师拼接；fundamental 成为执行来源；PM/Trader 解析自由触发文字或默认 `breakout`；共享 trigger/profile/source 不一致；缺 final contract；PM 自检失败；顶层 action/lots 或 `lots_delta` 不一致；当前触发未确认却写直执行权限 | no trade、rank 低、资金预算不足、信号弱、无合法执行证据的单品种合法取消候选 |
...
| `lifecycle_learning_trace.decision_learning_rows` | PM Step6 合约装配 | 最终 `final_action/current_lots/target_lots`、contract lifecycle、有效 action-value | Step6 final 决策层学习 rows | `final_action_contract.learning_used.pm_lifecycle_learning_trace.decision_learning_rows` | PM self-check、Reviewer、Researcher | PM self-check；pre-backtest PM 输出契约测试 | 复制 Step4 临时 router rows；open/rank 混入 hold/reduce/exit/execution/conditional_monitor；reduce_exit 混入 open/add/execution；hold 混入非 hold；conditional_monitor 混入非 conditional_monitor；trace lifecycle 与最终合约不一致 | 对应生命周期没有有效学习，允许 rows 为空且不阻断当日候选 |
...
| `opportunity_scorecard` | PM 内部状态判断，Step2–3 | SCC、证据融合、当前持仓、风险上下文 | Step2 只把无真实冲突的 SCC 唯一代表方向写成 `preferred_side/side_priority=1`；Step3 保留候选质量和内部状态分项 | 同一个 PM 内存状态；Step6 仅把矩阵登记摘要写入 `final_action_contract.evidence_used` | PM Step4/Step5、Reviewer、Researcher | contract coverage；PM 最终合约间接校验 | Step2 按机会分或学习重选方向；scorecard 独立输出 artifact、直接写最终 rank/手数/资金部署、替代 full-market rank、形成第二套交易计划 | 真实方向冲突、候选质量低、单来源低分或未获预算 |
...
| `rank_capital_layer_contract` | PM full-market capital deployment，Step5 | 当日所有实际增加风险的候选池、资金状态、rank score、资金政策 | rank、capital layer、ratio source、资金部署解释 | `final_action_contract.capital_deployment` | PM self-check、Trader Phase2 strategy scheduler（仅只读 `rank_budget_sequence`）、Reviewer、Researcher | PM self-check；pre-backtest PM 输出契约测试 | `open/open_probe/open_real`、同方向扩大绝对手数的 `add/scale` 或获非零条件目标的 watch 未经过 rank；rank 字段缺失；`risk_reduction_candidate` 被计入新增风险证据或 rank；`wait/hold/reduce/exit`、当前反转退出腿或 `target_lots=current_lots` 的零增量条件监控错误携带 rank；资金层字段缺失；Phase2 未按已签 `rank_budget_sequence` 调度新增风险；rank 写在非 PM artifact 里变成交易权限 | 未入选、资金层级低、资金利用不足 |
...
| `execution_contract` | Trader 执行入口，Phase2 | 审计通过的 final contract、盘中行情、执行配置 | 单张 FAC 的执行触发摘要、执行 profile、触发条件、失效条件；条件 FAC 用15分钟线确认并由下一根合法1分钟线执行，直执行 FAC 对所有合法 profile 跳过15分钟复判并使用合法1分钟线；策略新增风险在实际执行价和动态保证金率确定后、transaction 形成前复核账户总保证金硬线 | Trader runtime payload；transaction audit payload 摘要 | Trader、Reviewer、Researcher | pre-backtest artifact boundary；Trader trigger parity；daily PG 只核对已落地执行与成交事实 | 复制完整 PM 合约；包含 `learning_used`、rank、score、资金解释、PM 学习；用 rank 生成交易权限或改方向/手数；放宽 PM 触发；把直执行记成 Trader 再次检查触发；add/scale 重复计算已有持仓保证金；动态实际投影超过 `max_total_margin_ratio` 仍形成 transaction | 未触发、价格错过、流动性不足、实际动态保证金超线而以既有 `margin_insufficient` 未成交 |
...
| `alpha_setup_action_value` | Researcher 写入工具，Phase4 后 | open/add 只使用已结算完整策略 episode；hold/reduce/exit/execution 可使用对应日记录；未交易机会只作既有弱先验 | 候选 action-value：`action_name -> canonical_action_family -> action_value_lane/learning_lane -> action_preference`；只有字段完整、语义一致且具正式 preference 的记录可成为 PM canonical 学习 | `alpha_setup_action_value` DB；payload_json 同值保留 | PM next-day retrieval、Reviewer、Researcher | 普通跨日 episode→action-value→PM retrieval 回归；pre-backtest action matrix contract；daily PG 只检查实际生成记录的来源日期与前视边界 | 裸 transaction 或日 PnL 碎片生成 open/add reward；分批平仓覆盖；缺 canonical family/lane/preference；family/lane/preference 不一致；future dated；非策略/rollover/forced_risk 污染 strategy action-value | episode 没有形成正式 preference、observe 空 preference、样本少、reward 弱均合法 |
| `alpha_setup_profile` / product learning | Researcher 写入工具，Phase4 后 | 完整 episode 聚合交易次数/胜负/PnL；日记录补充非 open/add 生命周期与执行事实 | 产品/setup/trigger 历史表现 | research DB | 分析师校准、PM product learning、Researcher | contract coverage；data time boundary | 将一笔 episode 的每日 PnL 碎片重复计成多笔交易；直接写手数、交易授权、当日合约修改 | 产品表现差、setup 样本少 |
...
| `trade_episode_memory` / `no_trade_opportunity_memory` | Reviewer / Researcher，Phase4 后 | 已结算完整策略交易对、未交易机会、未触发条件机会 | 可为空的未来学习事实底座；episode 不是 formal action-value | research DB；artifact | Researcher；分析师与 PM 只消费后续按各自作用域形成的正式摘要/action-value，不直接读取 episode 表 | data time boundary；mechanism audit | 前视；未完成 Phase4/结算、裸 transaction、非策略、rollover 或 forced_risk 进入策略 episode 学习；要求每笔交易都形成学习 | 无合格 episode、学习为空、机会少、错过交易 |
