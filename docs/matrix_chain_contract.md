...
| `rank_capital_layer_contract` | PM full-market capital deployment，Step5 | Step4已定层且实际增加风险的候选、资金状态、到期预测校准和rank政策 | 八项有符号 `rank_score` 只求和一次：冷启动证据 + 层级6/3/0 + canonical open/add历史学习 + setup历史 + 当日SCC触发质量×0.08 + 校准预测价值 + 一次资金效率 - 风险；校准项只读取过去已到期的方向准确率、Brier、预测方向手续费后收益和作用域匹配。只按score降序、ticker排序生成 `rank_budget_sequence` | `final_action_contract.capital_deployment` | PM self-check、Trader只读预算顺序、Reviewer、Researcher | 普通PM回归；pre-backtest PM路径 | 原始LLM confidence冒充盈利概率；未到期预测进入Rank；第二排序；截断/负分禁入；非新增风险带rank | 合法负rank probe、预测冷启动、未获预算、同层证据或学习较弱 |
