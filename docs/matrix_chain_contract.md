...
| `alpha_setup_action_value` | Researcher 写入工具，Phase4 后 | open/add 每个已结算完整策略 episode 只提供一个聚合 reward，物理pair仅为经济明细；hold/reduce/exit/execution 可使用对应日记录；未交易机会只作既有弱先验 | 候选 action-value：`action_name -> canonical_action_family -> action_value_lane/learning_lane -> action_preference`；只有字段完整、语义一致且具正式 preference 的记录可成为 PM canonical 学习 | `alpha_setup_action_value` DB；payload_json 同值保留 | PM next-day retrieval、Reviewer、Researcher | 普通跨日 episode→action-value→PM retrieval 回归；pre-backtest action matrix contract；daily PG 只检查实际生成记录的来源日期与前视边界 | 裸 transaction 或日 PnL 碎片生成 open/add reward；一个episode的分批pair分别增加sample/trade/tail loss；缺 canonical family/lane/preference；family/lane/preference 不一致；future dated；非策略/rollover/forced_risk 污染 strategy action-value | episode 没有形成正式 preference、observe 空 preference、样本少、reward 弱均合法 |
...
| `adaptive_policy_state` / `provisional_policy_state` / `config_learning_overlay` | Researcher / 配置学习写入工具，Phase4 后 | 长窗口研究结果、验证通过的参数证据、回滚值；PM 生命周期校准只读 FAC 中的 `pm_lifecycle_learning_impact_delta` | 策略参数学习状态、候选参数、回滚信息；当前无合格参数优化生产者时不得复制原配置并标记为 overlay 刷新，既有原值复制 overlay 按原来源与原因精确停用 | research / config learning DB | 开发验收、PM 配置读取、PG | pre-backtest config consistency；PM 生命周期正式契约行为测试；data time boundary | 未验证参数直接生效；复制当前配置冒充已学习参数；从旧 PM 内部对象读取生命周期；缺 rollback；改当天交易事实；绕过 PM 合约权限；未来数据参与参数 | 候选策略待验证、样本不足、无合格参数优化结果时 overlay 写入为零 |
| `trade_episode_memory` / `no_trade_opportunity_memory` | Reviewer / Researcher，Phase4 后 | 已结算完整策略持仓周期及其物理pair经济明细、未交易机会、未触发条件机会；未交易身份的 `side/setup_type/entry_trigger/horizon_class/market_regime` 只继承对应 FAC | 可为空的未来学习事实底座；episode 不是 formal action-value；未交易 FAC 身份不完整时不写学习记录；多周期影子结果只作诊断，fast-candidate 仅使用固定周期内同作用域完整正负样本及未失效的 FAC 可执行依据，不再满足该证据的既有active政策同步停用 | research DB；artifact；`entry_trigger` 保存在既有 payload 与 `next_round_memory_contract.scope` | Researcher；分析师与 PM 只消费后续按各自作用域形成的正式摘要/action-value，不直接读取 episode 表 | data time boundary；mechanism audit；未交易身份 FAC 单源回归；fast-candidate 固定周期及正负样本回归 | 前视；未完成 Phase4/结算、裸 transaction、非策略、rollover 或 forced_risk 进入策略 episode 学习；未交易身份从分析师顺序、信号组合或推荐动作重建；挑选最佳影子周期、排除负样本、入场前已失效或缺 FAC 可执行依据的记录晋升政策；要求每笔交易都形成学习 | 无合格 episode、学习为空、机会少、错过交易、FAC 身份不完整而跳过未交易学习、固定周期样本不足而不晋升 |
