...
| `audit_verdict` | Auditor，Phase1 | PM final contract、账户状态、配置硬保证金上限、数据质量 | `approve` / `approve_with_warning` / `block`、hard/soft risk reasons、审计 payload | recommendation audit fields；audit artifact | Trader、Reviewer、PG | Auditor contract audit；daily PG 只核对审计与执行的外部事实 | Auditor 改方向、改手数、新建合约、直接消费研究记忆、复审 PM 学习/融合/rank/预算/sizing；审计通过缺 final contract；阻断后 Trader 仍执行 | 数据质量 warning/degraded、风险接近硬上限 |
