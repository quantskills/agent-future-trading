...
|---|---|---|---|---|---|---|---|---|
| `action_evidence_contract` | `technical` / `fundamental` / `commodity_news`，Phase1 | 截止点内行情、基本面、新闻、商品 profile、分析师校准摘要 | 三份经同一共享校验的结构化预测证据；technical 固定 `evidence_role=entry_timing`，可执行时机只允许 `breakout/pullback/vwap_confirmed`；fundamental 固定 `direction_context` 且不得形成 Trader profile；commodity_news 固定 `event_catalyst`，只有当前事件已满足即时边界时才可形成 `event_immediate` 候选。可执行 `entry_trigger` 由共享 canonical 定义按时机和方向确定性生成；`risk_reduction_candidate` 只服务已有持仓；必需市场事实不可用时仍由三个分析师各自产生合法中性 AEC | Workflow 在一个 Phase1 写事务中保存三份 signal 及全部 recommendation；`artifact_json`和分析师报告都只保存同一AEC；内存metadata保存前仅AEC、保存后仅AEC与真实ID | `signal_collector`、Reviewer、Researcher | 共享 AEC 校验；pre-backtest structured IO 与同库普通中性/数据不可用中性预演；daily PG 只核对实际进入策略路径时物理来源记录存在 | 含手数、仓位比例、保证金、`final_action_contract`、`opportunity_rank`、最终交易动作；角色/profile/trigger 不符合共享 canonical；fundamental 声明执行机会；news 声明普通15分钟 watch；自由执行文字进入正式 `entry_trigger`；Phase1失败后留下部分 signal/recommendation/artifact；伪造方向、profile、trigger、权限或市场事实 | 证据弱、证据冲突、基本面或新闻无新增、合法普通中性 `no_opportunity`、合法中性数据不可用状态 |
