...
| `rank_capital_layer_contract` | PM full-market capital deployment，Step5 | 当日所有实际增加风险的候选池、资金状态、rank score、资金政策 | rank、capital layer、ratio source、资金部署解释 | `final_action_contract.capital_deployment` | PM self-check、Reviewer、Researcher | PM self-check；pre-backtest PM 输出契约测试 | `open/open_probe/open_real`、同方向扩大绝对手数的 `add/scale` 或获非零条件目标的 watch 未经过 rank；rank 字段缺失；`risk_reduction_candidate` 被计入新增风险证据或 rank；`wait/hold/reduce/exit`、当前反转退出腿或 `target_lots=current_lots` 的零增量条件监控错误携带 rank；资金层字段缺失；rank 写在非 PM artifact 里变成交易权限 | 未入选、资金层级低、资金利用不足 |
...
| `execution_contract` | Trader 执行入口，Phase2 | 审计通过的 final contract、盘中行情、执行配置 | 执行触发摘要、执行 profile、触发条件、失效条件；条件 FAC 用15分钟线确认并由下一根合法1分钟线执行，直执行 FAC 对所有合法 profile 跳过15分钟复判并使用合法1分钟线 | Trader runtime payload；transaction audit payload 摘要 | Trader、Reviewer、Researcher | pre-backtest artifact boundary；Trader trigger parity；daily PG 只核对已落地执行与成交事实 | 复制完整 PM 合约；包含 `learning_used`、rank、score、资金解释、PM 学习；放宽 PM 触发；把直执行记成 Trader 再次检查触发；add/scale 重复计算已有持仓保证金；改方向、改手数 | 未触发、价格错过、流动性不足 |
