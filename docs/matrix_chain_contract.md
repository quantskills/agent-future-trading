...
| `rank_capital_layer_contract` | PM full-market capital deployment，Step5 | `current_lots=0` 且 `target_lots!=0` 的开仓候选池、资金状态、rank score、资金政策 | rank、capital layer、ratio source、资金部署解释 | `final_action_contract.capital_deployment` | PM self-check、Auditor、Reviewer、Researcher、PG | PM self-check；daily PG single trade truth | 开仓未经过 rank；rank 字段缺失；`add/scale/hold/reduce/exit/reverse` 当前合约错误携带 rank；资金层字段缺失；rank 写在非 PM artifact 里变成交易权限 | 未入选、资金层级低、资金利用不足 |
