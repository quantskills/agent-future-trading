...
运行机制：回测前检测只通过独立入口 `src/run/pre_backtest_test.py` 由操作者主动执行。操作者指定配置和日期窗口，查看就绪结论后自行决定是否启动 `src/run/backtest.py`；检测入口不得自动启动真实回测，`backtest.py` 也不调用、读取或保存回测前检测结果。
...
- 指定区间交易日能够解析；日线与首日合约事实的检查起点由正式交易日机制解析为 `Prev(start_date)`，正式回测交易日仍只属于用户指定区间，周末和节假日不要求存在日线。
- PandaAI 日线具备开盘价、收盘价、结算价和主力合约信息。
- 合约乘数、保证金率和具体合约信息可读取。
- Trader 使用的分钟行情接口及字段结构存在；回测前只从 Router 已解析的具体合约事实中确定一个代表合约，分别调用一次 15 分钟和 1 分钟接口，不按品种遍历分钟数据。
- 数据日期不得产生前视。
- PandaAI 的 HTTP 502、503、504、HTTP 429、正式 SDK 瞬时服务码及已识别瞬时网络错误只在适配器当前请求内持续退避重试，直到取得真实响应或操作者终止进程；鉴权、权限、参数、接口/频率不支持、数据结构错误及其他明确非瞬时错误不重试。
...
预演必须分别覆盖两类正式中性状态：三个分析师的数据可用普通 `Neutral` 经真实 finalization 收口为合法 `no_opportunity` AEC，并在同库继续贯通 SCC 至次日学习读取；必需市场事实不可用时的三份中性 AEC 继续调用同一共享校验器验证。两类确定性输入只属于隔离验收，不得成为生产默认事实或历史故障专用检查。
...
- 三类分析师角色与执行触发使用同一共享 canonical 校验：technical 只允许三种15分钟 profile，fundamental 不产生执行来源，commodity_news 只允许已确认的 `event_immediate`；PM 和 Trader 不解析自由触发文字、不默认 `breakout`。这是既有契约装配验收，不新增历史故障专用检查。
...
- 操作者必须在 reset 和任何正式业务写入之前独立完成回测前检测并查看结论；该先后责任不通过检测状态文件或 `backtest.py` 内嵌调用实现。
...
运行机制：既可通过独立入口 `src/run/backtest_daily_test.py` 单独执行，也由自动化回测入口 `src/run/backtest.py` 在每个交易日的 Phase1–4 与 Researcher 全部结束后调用。自动化回测对当前逻辑交易日只调用一次，检测通过后才进入下一交易日，失败立即停止；它只在单日业务链结束后运行，不进入 Phase1–4、Researcher 或任何智能体的内部运行阶段。
...
- 指定日期窗口时，数据就绪检查使用正式 `get_previous_trading_day` 从 `Prev(start_date)` 开始覆盖日线 OHLCV、结算价、主力映射、去重后的具体合约、乘数和保证金率，区间交易日仍只取用户指定窗口；分钟能力验收只选取窗口内第一个合法具体合约，对同一合约各调用一次 15 分钟与 1 分钟接口，并校验 `datetime/open/high/low/close/volume`、具体合约及逻辑交易日。物理时间位于前一日晚间但 `trading_date=T` 合法，未来逻辑交易日、字段缺失或合约错配硬失败；未指定窗口时不推定数据事实。
- PandaAI 瞬时网关、正式限流码和瞬时网络错误由正式适配器在当前请求内持续处理；数据就绪不维护第二套重试器，不跳过请求，也不返回空数据冒充成功。适配器的现有精确缓存只有在查询键与记录事实一致时才可命中，无效或错配缓存必须重新读取真实接口。
- 分钟能力失败在既有 `violation_codes[]` 中区分接口/频率不可用、接口运行失败、必需字段缺失、具体合约错配和逻辑交易日错误；报告只保存安全类别，不保存原始异常、请求头或凭据。
