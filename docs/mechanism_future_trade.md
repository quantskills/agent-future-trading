...
8. **最终交易真相只认结构化出口**：PM 自然语言只作为审计材料；新开仓、probe、放大、减仓、持有和退出必须落到顶层 `final_action_contract`、顶层 `final_new_entry_trade_authority`、`active_opportunity_audit`、Trader 执行结果和会计结算结果。普通策略单先由 PM 生成唯一 `final_action_contract`，经 Auditor 审核后再交给 Trader 执行。`pre_open_plan` 只保留为 PM 内部草稿和日志，不是成交、审计、评估或学习的兜底事实来源。`direction_only/watchlist/no_trade` 不能被最小手数、旧 probe seed 或软门控释放绕成真实开仓。

## 二、已经代码落地的实际业务运行机制

### 1. 四阶段业务流

- **Phase1 盘前策略**：分析师和 PM 生成每个品种的目标方向、目标仓位、推荐动作、目标手数、合约、参考价格、失效边界和审计快照。普通策略执行依据只能是顶层 `final_action_contract`、顶层 `final_new_entry_trade_authority` 和 `active_opportunity_audit`；`pre_open_plan` 只用于 PM 内部草稿留痕，不能成为 Trader 或 Researcher 的事实入口。
- **Phase2 盘中执行**：Trader 读取待执行推荐，按当前持仓和目标手数翻译为 `open_long`、`open_short`、`close_long`、`close_short` 或 `hold`。Trader 不能从 PM 文本、旧 probe 标记或最小一手机制自行创造策略方向，只能执行 PM/Auditor 已授权计划。
