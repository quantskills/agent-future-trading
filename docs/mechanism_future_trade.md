...
- 对 `trigger_confirmation_adjustment=stronger` 的入场，Trader 在既有 FAC 内验证初次触发后的下一根完整15分钟线；对 `strict` 验证连续两根完整15分钟线，且整段确认序列必须满足价格延续和相对量能。standard 候选不加该门；该确认只决定已审计入场 FAC 是否成交，不改变 PM 方向、手数、rank 或下一交易日唯一退出 FAC。
