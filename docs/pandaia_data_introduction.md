...
本文档说明 agent-future-trading 当前如何使用 PandaAI 数据，以及 PandaAI 在盘前分析、市场确认、盘中执行、运营风控、日终结算和回测验收中的角色。PandaAI 是行情与衍生数据源，不是交易授权来源；真实策略交易必须经过分析师结构化预测证据、信号收集员统一证据包、投资组合经理唯一 `final_action_contract`、审计员、交易员和会计师。
...
扩展接口的成功响应与确定性空响应按“接口名 + 完整请求参数”写入 `src/assets/pandaai_market_cache.db` 的 `pandaai_extra_data_cache` 表。新进程再次请求同一历史参数时直接读取本地缓存；权限、网络、额度和其他 provider 异常不写入缓存。原有 `pandaai_market_data_cache` 日线表原地保留，不需要删除或重建缓存数据库。
...
6. 分钟线仍由 Trader 在 PM 最终合约与 Auditor 放行后按实际合约、频率和 cutoff 调用，不进行15品种全量预下载。
...
回测前 `src/run/pre_backtest_test.py` 会统一编排回测前结构测试、协议预检、契约覆盖和回测前验收，并通过现有只读数据入口真实检查指定窗口与配置品种的数据就绪性。交易日、PandaAI 日线开盘价、收盘价、官方结算价、主力合约映射、合约乘数、保证金率、具体合约信息和 Trader 分钟行情接口能力属于交易必需数据，断裂时阻止回测；可选衍生数据缺口可以 warning 降级。Finoview 与新闻只检查路径、文件可读性、解析及日期过滤，某品种某日没有新增基本面或新闻不阻断。该检查不调用 LLM、不运行真实回测、不写正式业务库。回测过程中 `src/run/backtest_daily_test.py` 在每个交易日结束后只读取该日真实 DB、artifact 和 payload，只检查当日阶段、物理落点、唯一交易事实来源、策略单与运营单分账、审计放行与执行结果、成交、结算、账户和实际生成学习记录的时间边界。每日 PG 不读取 PM 自检，不检查 action-value 如何影响投资组合经理 score/rank/手数/动作，也不复查任何智能体内部机制；无交易、收益差、rank 低或学习为空进入策略效果分析。

PandaAI 返回 `500009`（单日总流量超限）时，数据适配器设置当日进程级阻断并抛出稳定错误码 `pandaai_daily_quota_exhausted`。Phase1 日线或扩展数据预取收到该错误后立即终止，不再遍历剩余品种；新进程在额度恢复后重新运行，已持久化的历史扩展参数继续从本地缓存读取。

## 八、当前限制

1. 分钟线执行是实战近似，不是盘口逐笔撮合。
2. 夜盘小节和交易日归属仍可继续细化。
3. 未缓存的新扩展参数仍需首次调用 PandaAI；精确参数缓存消除重跑产生的重复请求，不改变新交易日的数据需求。
4. `src/assets/pandaai_market_cache.db` 是日线与扩展数据缓存，不是回测交易记录；清空交易记录不需要删除它。
