...
本文档说明 AgentQuant 当前如何使用 PandaAI 数据，以及 PandaAI 在盘前分析、市场确认、盘中执行、运营风控、日终结算和回测验收中的角色。PandaAI 是行情与衍生数据源，不是交易授权来源；真实策略交易仍必须经过分析师证据、PM 唯一 `final_action_contract`、Auditor、Trader 和 Accountant。
...
| 场景 | 使用方 | 用途 | 边界 |
| --- | --- | --- | --- |
| 日频行情 | Technical、PM、Trader、Accountant、回测日历 | 技术分析、盘前参考价、主力合约、执行基准、结算审计 | Phase1 只用盘前可见数据，不能用当日结算反推策略 |
| 分钟线行情 | Trader、forced_risk 扫描 | 15m 判断触发，1m 选择执行基准价，盘中保证金风险估价 | 只能使用当时 cutoff 前已发生 bar；回测也不能用未来 bar 改写当时判断 |
| 期货衍生数据 | Fundamental、PM market confirmation | 基差、仓单、持仓排名、资金流、多空比等确认 | 缺失只能写 data quality，不能兜底成方向信号 |
| 合约详情 | Trader、Accountant | 合约乘数、最小变动价位、保证金率、涨跌停/交割保护 | 无数据时回退本地静态缓存，并记录来源 |
| 结算价 | Accountant | Phase3 逐日盯市、保证金重算、PnL | 只能收盘后使用，不参与当天 Phase1 策略 |
| 交易日识别 | Backtest、日历工具 | 解析回测交易日窗口 | 只决定运行日期，不提供交易方向 |
...
Technical 使用这些字段计算趋势、波动率、支撑阻力、RSI、MACD、ATR、均线等。PM 使用它们作为 market confirmation 和参考价来源。Trader 使用可见价格和限价字段做成交保护。Accountant 只在 Phase3 使用结算价做账。
...
- 15m K 线用于盘中触发确认。
- 1m K 线用于执行基准价。
- 条件 probe 只在 PM 合约授权后，由 Trader 用分钟线检查触发。
...
这些数据主要进入 Fundamental 和 PM 的 market confirmation。字段缺失、接口无权限、记录数不足或日期滞后，都必须进入 feature status / data quality；不能把缺数据包装成 Bullish/Bearish，也不能单独授权开仓。

### 4. 合约信息和保证金

Trader 和 Accountant 会读取合约代码、合约乘数、最小变动价位、保证金率和交易所信息。保证金率优先使用 PandaAI 合约详情；若不可用，回退本地静态合约缓存。所有回退都必须写入审计信息。
...
4. Phase4 研究结果只能写给未来交易日，不能回改当日推荐、成交或账务。
...
普通策略单必须来自审计后的 `final_action_contract`。条件 probe 必须在合约里写明 `conditional_trigger_authority=true`、触发条件和失效边界；Trader 只用 PandaAI 分钟线判断触发是否成立，不能自己创造方向、手数或保证金目标。
...
回测前 `pre_backtest_acceptance.py` 会检查数据、配置、字段语义、协议边界和本地库状态。回测过程中 `system_invariant_audit.py` 会逐日累计检查唯一合约、字段语义、未完成交易日、策略单与运营单分账、Trader 执行是否只来自最终合约。PandaAI 数据缺口可以作为 warning 降级，但若导致未来函数、账务错误或交易合约冲突，必须 hard fail。
