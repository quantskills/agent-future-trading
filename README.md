...
公开发布说明：上述行情、基本面和新闻目录仅是运行时接口约定，公开仓库不附带实际数据文件；请使用自己合法取得的数据。
...
本公开版本只提供代码、配置模板和最小测试入口。私有验收清单、逐笔回测结果、工作日志和策略研究文档不随仓库发布。
...
- 普通持仓 -2%/-4% 亏损复核、所有仓位类型盈利完全回吐后的既有减仓复核是否按统一口径命中。
